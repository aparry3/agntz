...
- **@agntz/stores / memrez**: migrated onto `@agntz/db` for pool creation and migrations. Table ownership is unchanged (`ar_*` vs `memrez_*`) and behavior is preserved. memrez's Postgres store additionally gains the advisory-locked, reset-on-failure migration path, fixing its latent poisoned-promise bug.
