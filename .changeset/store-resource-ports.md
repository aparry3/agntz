...
- **@agntz/stores** now depends on `@agntz/contracts` instead of `@agntz/core` — its memory, Postgres, and SQLite implementations use the store ports without pulling in the runtime (no more transitive AI-SDK provider deps).
