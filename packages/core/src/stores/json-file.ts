...
import { decodeTraceCursor, encodeTraceCursor } from "./memory.js";
...
export class JsonFileStore implements UnifiedStore {
	...
	private userRoot(): string {
		return join(
			this.basePath,
			"users",
			this.sanitizeFilename(this.requireUser()),
		);
	}
	...
	private async readAllVersions(
		agentId: string,
	): Promise<StoredAgentVersion[]> {
	...
	async putAgent(agent: AgentDefinition): Promise<void> {
		...
		await this.writeJson(
			join(dir, `${this.filenameSafeTimestamp(now)}.json`),
			stored,
		);
	}
	...
	private async readAliases(agentId: string): Promise<Record<string, string>> {
		return (
			(await this.readJson<Record<string, string>>(
				this.aliasFilePath(agentId),
			)) ?? {}
		);
	}
	...
	async getAgentVersion(
		...
		const path = join(
			...
			`${this.filenameSafeTimestamp(createdAt)}.json`,
		);
	...
	private sessionPath(sessionId: string): string {
		return join(
			this.userRoot(),
			"sessions",
			`${this.sanitizeFilename(sessionId)}.json`,
		);
	}
	...
	private contextPath(contextId: string): string {
		return join(
			this.userRoot(),
			"context",
			`${this.sanitizeFilename(contextId)}.json`,
		);
	}
	...
	async getSecretValue(name: string): Promise<string | null> {
		await this.ensureUserDirs();
		const row = await this.readJson<{ encrypted: string }>(
			this.secretPath(name),
		);
		if (!row) return null;
	...
	private spanPath(spanId: string): string {
		return join(
			...
			"traces",
			"spans",
			`${this.sanitizeFilename(spanId)}.json`,
		);
	}
	...
	async insertSpansBatch(spans: Span[]): Promise<void> {
		await this.ensureTraceDirs();
		await Promise.all(
			spans.map((s) => this.writeJson(this.spanPath(s.spanId), { ...s })),
		);
	}
	...
	async updateSpan(
		...
		await this.writeJson(this.spanPath(spanId), {
			...existing,
			...patch,
			spanId,
			ownerId,
	...
	async deleteTrace(traceId: string, ownerId: string): Promise<void> {
		const summary = await this.readJson<TraceSummary>(
			this.summaryPath(traceId),
		);
	...
	async deleteOlderThan(ownerId: string, before: Date): Promise<number> {
		...
		for (const file of files) {
			if (!file.endsWith(".json")) continue;
			const s = await this.readJson<TraceSummary>(join(summariesDir, file));
			if (s && s.ownerId === ownerId && s.startedAt < beforeIso)
				toDelete.push(s.traceId);
		}
		const spansDir = join(this.basePath, "traces", "spans");
	...
	async revokeApiKey(params: { userId: string; keyId: string }): Promise<void> {
		const rows = await this.readApiKeys();
		const row = rows.find(
			(r) => r.id === params.keyId && r.userId === params.userId,
		);
		if (!row) return;
	...
	private async readWebhookDeliveries(): Promise<WebhookDelivery[]> {
		return (
			(await this.readJson<WebhookDelivery[]>(this.webhookDeliveriesPath())) ??
			[]
		);
	}
