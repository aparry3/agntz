// ═══════════════════════════════════════════════════════════════════════
// Agent reference parser — `<id>[@<version|latest>]`
// ═══════════════════════════════════════════════════════════════════════
//
// Reference forms:
...
export interface ParsedAgentRef {
  agentId: string;
  /** `"latest"`, an ISO 8601 timestamp, or undefined (use activated). */
  version?: string;
}
...
export function parseAgentRef(input: string): ParsedAgentRef {
  ...
  if (version !== "latest" && !isIsoTimestamp(version)) {
    throw new InvalidAgentRefError(
      input,
      `version must be "latest" or an ISO 8601 timestamp (got "${version}")`,
    );
  }
