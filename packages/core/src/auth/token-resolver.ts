...
function normalizeOAuth2(a: OAuth2ClientCredentialsAuth): NormalizedAuth {
  ...
  return {
    ...
    apply: normalizeApply({ location: "header", name: "Authorization", format: "Bearer {token}" }),
...
function normalizeApply(apply: TokenExchangeApply | undefined): Required<TokenExchangeApply> {
  const location = apply?.location ?? "header";
  const name = apply?.name ?? (location === "header" ? "Authorization" : "");
  const format = apply?.format ?? (location === "header" ? "Bearer {token}" : "{token}");
...
function stableStringify(node: unknown): string {
  ...
  return `{${keys
    .map((k) => `${JSON.stringify(k)}:${stableStringify((node as Record<string, unknown>)[k])}`)
    .join(",")}}`;
}
...
async function fetchToken(
  ...
  const bodyType = auth.request.body_type ?? (auth.request.body !== undefined ? "json" : undefined);
  ...
  if (basicAuthHeader === "Basic __OAUTH2_BASIC__") {
    const cid = interpolate((auth.shape as OAuth2ClientCredentialsAuth).client_id, state);
    const csec = interpolate((auth.shape as OAuth2ClientCredentialsAuth).client_secret, state);
  ...
  if (isText) {
    ...
    if (!auth.extract.token_path) {
      throw new AuthError("token_exchange.extract.token_path is required for JSON responses");
    }
    const tokenVal = jsonPathGet(parsed, auth.extract.token_path);
    if (tokenVal == null || typeof tokenVal !== "string" || tokenVal.length === 0) {
      throw new AuthError(
...
function jsonPathGet(node: unknown, path: string): unknown {
  ...
  while ((m = re.exec(rest)) !== null) {
    if (m.index !== cursor) {
      throw new AuthError(`Unsupported JSONPath syntax near '${rest.slice(cursor)}'`);
    }
