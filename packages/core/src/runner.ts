import { formatAgentRef, isIsoTimestamp, parseAgentRef } from "./agent-ref.js";
...
import { DEFAULT_REPLY_MAX_PER_RUN, isContentBlockArray } from "./types.js";
...
function resolveCallerTightensLimit(
	...
	if (typeof optionVal === "number" && optionVal > 0)
		candidates.push(optionVal);
...
function clampResourceMode(
	declared: ResourceMode,
	parent: ResourceMode | undefined,
): ResourceMode {
...
export class Runner {
	...
	constructor(config: RunnerConfig = {}) {
		...
		this._providerStore =
			unifiedStore && "getProvider" in unifiedStore
				? (unifiedStore as ProviderStore)
				: undefined;
		this._connectionStore =
			unifiedStore && "getConnection" in unifiedStore
				? (unifiedStore as ConnectionStore)
				: undefined;
		this._skillStore =
			unifiedStore && "getSkill" in unifiedStore
				? (unifiedStore as SkillStore)
				: undefined;
		this._secretStore =
			unifiedStore && "getSecretValue" in unifiedStore
	...
	get providers(): ProviderStore | undefined {
		return this._providerStore;
	}
	/** Access the connection store (if available) */
	get connections(): ConnectionStore | undefined {
	...
	get tokenResolver(): TokenResolver {
	...
	private resolveResourcesForAgent(
		...
		modesByKind: Record<string, ResourceMode>;
	} {
		this.assertResourceProviders(agent);
		...
		for (const [name, definition] of Object.entries(agent.resources ?? {})) {
			...
			const mode = clampResourceMode(
				declaredMode,
				parentModes?.[definition.kind],
			);
	...
	private async resolveAgent(
		...
		if (!isIsoTimestamp(ref.version)) {
			...
			const stored = await this.agentStore.getAgentVersion(
				ref.agentId,
				aliasTarget,
			);
			if (!stored) {
	...
	stream(
		...
		async function* generate(): AsyncGenerator<StreamEvent> {
			...
			const maxDepth =
				self.config.maxRecursionDepth ?? DEFAULT_MAX_RECURSION_DEPTH;
			...
			if (runRegistry) {
				if (!runId) {
					...
					if (isTopLevel) {
						const release =
							await runRegistry.acquireSessionLock(effectiveSessionId);
						try {
							const activeRunId =
								runRegistry.findActiveBySession(effectiveSessionId);
							if (activeRunId) {
			...
			const loadedSkillToolDescriptors: Array<{
				name: string;
				description: string;
			...
			const modelConfig = {
				...
				...agent.model,
				temperature:
					agent.model.temperature ?? self.config.defaults?.temperature,
			...
			try {
				...
				const normalizedInput: string | ContentBlock[] = isContentBlockArray(
					input,
				)
				...
				while (step < effectiveMaxSteps) {
					...
					if (
						...
						totalUsage.totalTokens >= tokenBudget
					) {
				...
				if (!effectiveSignal?.aborted) {
					...
					const skipEmptyAssistant =
						replyEnabled && !finalOutput && replyCollector.length > 0;
	...
	async invoke(
		...
		const currentDepth = options._recursionDepth ?? 0;
		const maxDepth =
			this.config.maxRecursionDepth ?? DEFAULT_MAX_RECURSION_DEPTH;
		if (currentDepth > maxDepth) {
			throw new MaxRecursionDepthError(agentId, maxDepth);
		}
			...
			while (step < effectiveMaxSteps) {
				...
				if (runRegistry && rootId && runId) {
					runRegistry.emit(rootId, {
						...
						runId,
						step,
						toolCalls: allToolCalls.slice(
							allToolCalls.length - result.toolCalls.length,
						),
						seq: 0,
					});
				}
	...
	private async executeToolCall(params: {
		...
		loadedSkillToolDescriptors: Array<{
			name: string;
			description: string;
			parameters: Record<string, unknown>;
		}>;
		...
		const toolCtx: ToolContext = {
			...
			registerSkillTools: async (refs) => {
				const descriptors = await this.toolRegistry.registerToolReferences(
					refs,
					{
						resolveAgentAsTool: (id) => this.resolveAgentAsTool(id),
						resolveMCPTools: (server, tools) =>
							this.resolveMCPTools(server, tools),
						ensureMCPServerRegistered: (s) => this.ensureMCPServerRegistered(s),
					},
				);
				for (const d of descriptors) {
					if (
						!loadedSkillToolDescriptors.some(
							(existing) => existing.name === d.name,
						)
					) {
		...
		try {
			...
			if (
				err instanceof MaxRecursionDepthError ||
		...
		if (hasResources) {
			...
			for (const resource of resources) {
				...
				for (const providerTool of providerTools) {
					...
					const toolName = makeResourceToolName(
						resource.name,
						providerTool.name,
					);
					if (
						addedResourceToolNames.has(toolName) ||
					...
					const tool: ToolDefinition = {
						...
						execute: async (input: unknown) => {
							return providerTool.execute(
								input,
								this.makeResourceToolContext(resource, opts?.grants ?? [], {
									agentId: agent.id,
	...
	private resolveAgentAsTool(agentRef: string): {
		...
		const sanitized =
			`${parsed.agentId}${parsed.version ? `__${parsed.version}` : ""}`.replace(
				/[^a-zA-Z0-9_]/g,
				"_",
			);
