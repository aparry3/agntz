...
function buildHttpUrl(
	...
	const pathTemplate =
		queryStart === -1 ? urlTemplate : urlTemplate.slice(0, queryStart);
	const queryTemplate =
		queryStart === -1 ? "" : urlTemplate.slice(queryStart + 1);
			...
			const substituted = rawVal.replace(
				...
				(_full, rawName: string, opt: string | undefined) => {
					...
					if (v == null) {
						if (opt === "?") return "";
						throw new Error(
							`Missing required placeholder '${rawName}' for URL`,
						);
					}
					return v;
				},
...
export function buildHttpToolDefinition(
	...
	const bodyType =
		entry.body_type ?? (entry.body !== undefined ? "json" : undefined);
...
async function runRequest(args: RunRequestArgs): Promise<unknown> {
	...
	const finalUrl = appliedQuery
		? appendQuery(args.url, appliedQuery)
		: args.url;
	const outboundUrlPolicy =
		args.deps.outboundUrlPolicy ??
	...
	if (
		args.auth &&
		args.allowRefresh &&
		args.refreshStatuses.has(response.status)
	) {
		await args.deps.tokenResolver?.invalidate(
			args.auth,
			args.deps.authCtx ?? {},
		);
