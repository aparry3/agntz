...
export async function runEval(
	...
	for (let i = 0; i < testCases.length; i++) {
		...
		try {
			...
			const totalWeight =
				assertionResults.reduce((sum, _, idx) => {
...
async function runCustomAssertion(
	...
	if (
		...
		"plugin" in assertion.value
	) {
		// { plugin: "myAssertion", ...rest }
		const { plugin, ...rest } = assertion.value as {
			plugin: string;
			[key: string]: unknown;
		};
		pluginName = plugin;
		...
		return {
			...
			score: 0,
			reason:
				'Custom assertion value must be a string (plugin name) or an object with { plugin: "name", ... }',
		};
	}
...
function assertJsonSchema(output: string, schema: object): AssertionResult {
	try {
