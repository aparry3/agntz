import type { AiMessagePart, AiSdkMessage } from "./message-builder.js";
import type {
	Span,
	SpanKind,
	TokenUsage,
...
export class SpanEmitter {
	...
	constructor(config?: TelemetryConfig) {
		...
		if (config?.tracer) {
			...
			this.tracer = api
				? api.trace.getTracer(config.tracerName ?? "agntz")
				: null;
		} else {
	...
	startRun(params: {
		...
		if (params.requestedVersion != null)
			attrs["agent.requested_version"] = params.requestedVersion;
		if (params.resolvedVersion != null)
			attrs["agent.resolved_version"] = params.resolvedVersion;
	...
	startInvoke(params: {
		...
		if (params.contextIds?.length)
			attrs["agent.context.ids"] = params.contextIds.join(",");
		if (this.config.recordIO && params.input)
			attrs["agent.input"] = params.input.slice(0, 4096);
		if (params.requestedVersion != null)
			attrs["agent.requested_version"] = params.requestedVersion;
		if (params.resolvedVersion != null)
			attrs["agent.resolved_version"] = params.resolvedVersion;
		...
		return {
			modelCall: (mp) => this.startModelCallInternal(mp, span),
			toolCall: (tp) => this.startToolCallInternal(tp, span),
			setResult: (result) => this.setInvokeResultInternal(span, result),
			end: () => this.closeSpan(span, "ok"),
			error: (err) => this.closeSpan(span, "error", err),
		};
	}
	...
	private startModelCallInternal(
		params: { model: string; step: number },
		parent: SpanState,
	): ModelCallSpan {
	...
	private openSpan(kind: SpanKind, name: string, opts: OpenOpts): SpanState {
		...
		const state: SpanState = {
			...
			attrs: opts.attrs ?? {},
			otel: this.tracer
				? this.tracer.startSpan(name, { attributes: opts.attrs ?? {} })
				: null,
		};
	...
	private setInvokeResultInternal(
		state: SpanState,
		result: {
			output?: string;
			usage: TokenUsage;
		...
		state.attrs["agent.usage.completion_tokens"] =
		...
		if (this.config.recordIO && result.output)
			state.attrs["agent.output"] = result.output.slice(0, 4096);
		if (state.otel) {
			for (const [k, v] of Object.entries(state.attrs))
				state.otel.setAttribute(k, v as string | number | boolean);
		}
	}
	...
	private setModelResult(
		...
		if (typeof r.costUsd === "number")
			state.attrs["agent.cost_usd"] = r.costUsd;
		if (this.config.recordIO) {
			if (r.prompt) {
				state.attrs["agent.prompt"] = JSON.stringify(
					sanitizeMessagesForTrace(r.prompt),
				);
			}
			if (r.completion) {
	...
	private setToolResult(state: SpanState, record: ToolCallRecord): void {
		...
		if (this.config.recordToolIO) {
			state.attrs["agent.tool.input"] = JSON.stringify(record.input).slice(
				0,
				4096,
			);
			state.attrs["agent.tool.output"] = JSON.stringify(record.output).slice(
				0,
				4096,
			);
		}
...
function stateToSpan(
	...
	return {
		...
		scores: {},
		costUsd:
			typeof s.attrs["agent.cost_usd"] === "number"
				? (s.attrs["agent.cost_usd"] as number)
				: null,
	};
}
...
function sanitizeMessagesForTrace(messages: AiSdkMessage[]): Array<{
	...
	content:
		| string
		| Array<{ type: string; text?: string; mediaType?: string; size?: number }>;
}> {
...
function sanitizePart(part: AiMessagePart): {
	type: string;
	text?: string;
	mediaType?: string;
	size?: number;
