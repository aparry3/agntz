...
export function defineSkill(definition: SkillDefinition): SkillDefinition {
	if (typeof definition.name !== "string" || !NAME_RE.test(definition.name)) {
		throw new Error(`Skill name must match ${NAME_RE} (lowercase-kebab-case)`);
	}
	if (
...
function validateToolReference(ref: ToolReference, index: number): void {
	...
	if (r.type === "inline") {
		if (typeof r.name !== "string" || r.name === "") {
			throw new Error(
				`Skill tools[${index}] (inline) requires a non-empty 'name'`,
			);
		}
	} else if (r.type === "mcp") {
		if (typeof r.server !== "string" || r.server === "") {
			throw new Error(
				`Skill tools[${index}] (mcp) requires a non-empty 'server'`,
			);
		}
		if (r.tools !== undefined && !Array.isArray(r.tools)) {
			throw new Error(
				`Skill tools[${index}] (mcp) 'tools' must be a string array when present`,
			);
		}
	} else if (r.type === "agent") {
		if (typeof r.agentId !== "string" || r.agentId === "") {
			throw new Error(
				`Skill tools[${index}] (agent) requires a non-empty 'agentId'`,
			);
		}
	} else {
		throw new Error(
			`Skill tools[${index}] has unknown type "${String(r.type)}"`,
		);
	}
}
