...
export async function resolveSpawnable(
	...
	for (const ref of spawnable) {
		if (ref.kind === "ref") {
			...
			const summary = def.description ?? def.systemPrompt.slice(0, 120);
...
export function createSpawnAgentTool(
	...
	return {
		...
		input: z.object({
			agent_id: z
				.enum(idEnum)
				.describe("Which sub-agent to invoke (must be from the allowlist)."),
			input: z
				.string()
				.describe("The task / question / input to send to the sub-agent."),
		}),
		async execute(input, ctx) {
			...
			if (!registry || !parentRunId) {
				return {
					ok: false,
					error:
						"spawn_agent requires the runner to be wired with a runRegistry; parent has no runId.",
				};
			}
			...
			if (!ids.includes(agent_id)) {
				return {
					ok: false,
					error: `agent_id "${agent_id}" is not in this agent's spawnable allowlist.`,
				};
			}
	...
	return {
		...
		input: z.object({
			run_ids: z
				...
				.optional()
				.describe(
					"Specific run_ids to query. If omitted, returns all your spawned children.",
				),
		}),
		async execute(input, ctx) {
			...
			if (!registry || !parentRunId) {
				return {
					ok: false,
					error: "check_agents requires a runRegistry on the parent Run.",
				};
			}

			const candidates = run_ids
				? (run_ids.map((id) => registry.get(id)).filter(Boolean) as Array<
						NonNullable<ReturnType<typeof registry.get>>
					>)
