import { z } from "zod";
import type { ToolContext, ToolDefinition, ToolReference, SkillStore } from "../types.js";
 ...
 */
export function createUseSkillTool(skillNames: string[]): ToolDefinition | null {
  ...
  return {
    ...
    description,
    input: z.object({
      skill: z.enum(names).describe("Which skill to load (must be from this agent's allowlist)."),
    }),
    async execute(input, ctx) {
      ...
      const register = c.registerSkillTools as
        | ((refs: ToolReference[]) => Promise<Array<{ name: string; description: string; parameters: Record<string, unknown> }>>)
        | undefined;

      if (!loaded || !store || !register) {
        return { ok: false, error: "use_skill requires the runner to thread skillStore, loadedSkills, and registerSkillTools through ToolContext." };
      }
