...
describe("Runner", () => {
	...
	it("registers and invokes an agent", async () => {
		const provider = new MockModelProvider(
			mockResponse("Hello from the agent!"),
		);
	...
	it("injects context into messages", async () => {
		const provider = new MockModelProvider(
			mockResponse("Based on the research..."),
		);
	...
	it("resolves agent-as-tool references", async () => {
		...
		const provider = new MockModelProvider([
			...
			mockResponse(
				"Article: TypeScript is a typed superset of JavaScript, created by Microsoft.",
			),
		]);
		...
		runner.registerAgent(
			defineAgent({
				...
				name: "Writer",
				systemPrompt:
					"Write articles. Use the researcher to gather facts first.",
				model: { provider: "openai", name: "gpt-5.4" },
...
describe("Runner — HTTP tools", () => {
	...
	afterEach(() => {
		if (restoreKey === undefined) {
			process.env.AGNTZ_SECRET_KEY = undefined;
		} else {
		...
		const provider = new MockModelProvider([
			{
				text: "",
				toolCalls: [
					{ id: "tc_1", name: "http__echo", args: { value: "hello" } },
				],
