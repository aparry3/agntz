...
describe("multi-agent spawning", () => {
  ...
  it("drain phase forbids parent from finishing while children are still running", async () => {
    ...
    const result = await runner.invoke("parent", "go", { runRegistry: registry });
  ...
  it("limits concurrent children — extra spawns return an error tool result", async () => {
    ...
    const runner = createRunner({
      modelProvider: new ScriptedModelProvider([
        // child blocks forever (until parent aborts) so the slot stays full
        {
          ...
          delayMs: 5000,
          respond: () => ({ text: "shouldn't reach", usage, finishReason: "stop" }),
        },
      ]),
    });
    const registry = new InMemoryRunRegistry();
    ...
    const { createSpawnAgentTool } = await import("../src/tools/spawn-agent.js");
    ...
    const r1 = (await tool.execute({ agent_id: "parked", input: "1" }, ctx as any)) as any;
    const r2 = (await tool.execute({ agent_id: "parked", input: "2" }, ctx as any)) as any;
    expect(r1.run_id).toBeDefined();
    expect(r2.run_id).toBeDefined();

    // Third spawn rejected by the gate.
    const r3 = (await tool.execute({ agent_id: "parked", input: "3" }, ctx as any)) as any;
  ...
  it("cancellation cascades from parent to children", async () => {
    ...
    const handle = (await tool.execute({ agent_id: "child", input: "go" }, ctx as any)) as any;
  ...
  it("check_agents returns only this parent's direct children", async () => {
    ...
    const check = createCheckAgentsTool([{ agentId: "fast", summary: "fast" }])!;
    ...
    await spawn.execute({ agent_id: "fast", input: "a" }, mkCtx(parentA.id) as any);
    await spawn.execute({ agent_id: "fast", input: "b" }, mkCtx(parentB.id) as any);

    // Wait for both to complete.
    await new Promise((r) => setTimeout(r, 50));

    const aChildren = (await check.execute({}, mkCtx(parentA.id) as any)) as any[];
    expect(aChildren.length).toBe(1);
    expect(aChildren[0].status).toBe("completed");

    const bChildren = (await check.execute({}, mkCtx(parentB.id) as any)) as any[];
