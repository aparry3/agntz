...
import { ImageFetchError, normalizeImageBlocks } from "../src/image-fetcher.js";
...
describe("normalizeImageBlocks", () => {
	...
	it("caches duplicate URLs — same image referenced twice fetches once", async () => {
		...
		const first = out[1] as Extract<
			ContentBlock,
			{ type: "image"; base64: string }
		>;
		const second = out[3] as Extract<
			ContentBlock,
			{ type: "image"; base64: string }
		>;
