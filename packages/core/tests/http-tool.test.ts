...
describe("buildHttpToolDefinition — body and method", () => {
	...
	it("attaches auth headers from the token resolver before fetch", async () => {
		const responses = [
			// 1. token endpoint
			new Response(
				JSON.stringify({ access_token: "tok-xyz", expires_in: 60 }),
				{
					status: 200,
					headers: { "content-type": "application/json" },
				},
	...
	it("refreshes the token once on 401 and retries", async () => {
		...
		const retryHeaders = (fetchMock.mock.calls[3][1] as RequestInit)
	...
	it("scrubs known tokens and secrets from response bodies", async () => {
		const cache = new MapTokenCache();
		cache.set("k", {
			token: "sekret-tok-1234567890",
			expiresAt: Date.now() + 60_000,
		});
		vi.spyOn(globalThis, "fetch").mockResolvedValue(
			new Response(
				JSON.stringify({
					message: "invalid: sekret-tok-1234567890 also static-secret-abc",
				}),
	...
	it("scrubs sensitive values from 4xx error response bodies", async () => {
		vi.spyOn(globalThis, "fetch").mockResolvedValue(
			new Response("error: leaked-secret-xyz-12345 is invalid", {
				status: 401,
			}),
		);
		const tool = buildHttpToolDefinition(
		...
		const result = (await tool.execute({}, noopCtx)) as {
			error: string;
			body: string;
		};
