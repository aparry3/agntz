...
function makeSkillStore(skills: SkillDefinition[]): SkillStore {
  const byName = new Map<string, SkillDefinition>(skills.map((s) => [s.name, s]));
  return {
    async getSkill(name) {
...
function makeCtx(
  overrides: {
    ...
    registerSkillTools?:
      | ((refs: ToolReference[]) => Promise<Array<{
          name: string;
          description: string;
...
describe("createUseSkillTool", () => {
  ...
  it("description lists each skill name", () => {
    const tool = createUseSkillTool(["researcher", "summarizer", "fact-checker"]);
  ...
  describe("execute", () => {
    ...
    it("loads a known skill: returns instructions and calls registerSkillTools with the skill's tools", async () => {
      ...
      expect(register).toHaveBeenCalledWith([{ type: "inline", name: "web_search" }]);
