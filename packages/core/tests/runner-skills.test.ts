...
describe("Runner integration with skills", () => {
  it("registers use_skill on turn 0, then skill tools after loading, and the system prompt advertises skills", async () => {
    ...
    const provider = new MockModelProvider([
      ...
      },
      {
        text: "",
        toolCalls: [
          { id: "tu_2", name: "web_search", args: { q: "test" } },
        ],
        usage: { promptTokens: 5, completionTokens: 1, totalTokens: 6 },
        finishReason: "tool-calls",
      },
    ...
    const result = await runner.invoke("test-agent", "Help me research X.", { userId: "u1" });
  ...
  it("the same skill is loaded only once across multiple use_skill calls in a single run", async () => {
    ...
    const provider = new MockModelProvider([
      {
        text: "",
        toolCalls: [{ id: "tu_1", name: "use_skill", args: { skill: "researcher" } }],
        usage: { promptTokens: 5, completionTokens: 1, totalTokens: 6 },
        finishReason: "tool-calls",
      },
      {
        text: "",
        toolCalls: [{ id: "tu_2", name: "use_skill", args: { skill: "researcher" } }],
        usage: { promptTokens: 5, completionTokens: 1, totalTokens: 6 },
        finishReason: "tool-calls",
      },
      {
    ...
    expect(result.toolCalls[1].output).toMatchObject({ alreadyLoaded: true, name: "researcher" });
  });
});
