...
describe("runEval", () => {
	...
	it("handles invoke errors gracefully", async () => {
		...
		expect(result.testCases[0].assertions[0].reason).toContain(
			"Model exploded",
		);
	});
...
describe("assertions", () => {
	...
	describe("llm-rubric (with mock model)", () => {
		it("uses model provider to judge output", async () => {
					...
					testCases: [
						{
							input: "test",
							assertions: [
								{ type: "llm-rubric", value: "Be friendly and warm" },
							],
						},
					],
				},
			...
			expect(result.testCases[0].assertions[0].reason).toBe(
				"Very friendly tone",
			);
		});
	});
	...
	describe("expectedOutput fallback", () => {
		it("uses contains assertion when expectedOutput is set with no assertions", async () => {
			const agent = makeAgent({
				eval: {
					testCases: [{ input: "test", expectedOutput: "world" }],
				},
			});
