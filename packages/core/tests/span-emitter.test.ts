...
describe("SpanEmitter", () => {
	it("startInvoke emits span-start with traceId and no parent", () => {
		...
		const start = events[0] as {
			type: string;
			span: { traceId: string; parentId: string | null; kind: string };
		};
	...
	it("nested manifest > invoke threads parentId and shares traceId", () => {
		const { emitter, events } = withEmitter();
		const m = emitter.startManifest({
			ownerId: "u1",
			agentId: "a1",
			kind: "llm",
		});
		const inv = emitter.startInvoke({
			agentId: "a1",
			invocationId: "i1",
			model: "m",
			ownerId: "u1",
		...
		inv.end();
		m.end();
		const starts = events.filter((e) => e.type === "span-start") as Array<{
			span: {
				traceId: string;
	...
	it("model.call and tool.execute spans nest under invoke as explicit children", () => {
		...
		inv.end();
		const starts = events.filter((e) => e.type === "span-start") as Array<{
			span: { kind: string; parentId: string | null; spanId: string };
		}>;
	...
	it("span-end carries patch with status, duration, error", () => {
		...
		const end = events.find((e) => e.type === "span-end") as {
			type: string;
			patch: {
				status: string;
				error: string | null;
