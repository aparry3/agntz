...
describe("Typed Errors", () => {
	...
	it("throws InvocationCancelledError when signal is aborted", async () => {
		...
		try {
			...
			expect((err as InvocationCancelledError).code).toBe(
	...
	it("AgentVersionNotFoundError exposes agentId and version", () => {
		const err = new AgentVersionNotFoundError(
			"reviewer",
...
describe("Resource limits", () => {
	...
	class LoopingProvider implements ModelProvider {
		...
		constructor(
			private tokensPerCall: number,
			private toolName: string | null = "noop",
		) {}
		async generateText(
			options: GenerateTextOptions,
		): Promise<GenerateTextResult> {
			...
			return {
				...
				usage: {
					promptTokens: half,
					completionTokens: this.tokensPerCall - half,
	...
	function registerLoopAgent(
		runner: ReturnType<typeof createRunner>,
		opts: { maxSteps?: number; tokenBudget?: number; timeoutMs?: number } = {},
	) {
		runner.registerAgent(
			defineAgent({
				id: "looper",
	...
	it("AgentDefinition.timeoutMs alone triggers InvocationTimeoutError", async () => {
		...
		await expect(runner.invoke("sleeper", "go")).rejects.toBeInstanceOf(
			InvocationTimeoutError,
		);
	});
