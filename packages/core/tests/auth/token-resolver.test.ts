...
describe("createTokenResolver — token_exchange", () => {
	...
	it("interpolates {{secrets.X}} in the token request body", async () => {
		...
		await resolver.resolve(
			baseAuth,
			{ secrets: { user: "alice", pass: "swordfish" } },
			{},
		);
	...
	it("supports apply.location=query", async () => {
		const fetchImpl = mockFetch([
			{ body: JSON.stringify({ access_token: "qtok" }) },
		]);
		const auth: HTTPAuth = {
			...baseAuth,
	...
	it("dedupes concurrent token fetches (single-flight)", async () => {
		...
		const gated = new Promise<string>((res) => {
			resolveOuter = res;
		});
		const fetchImpl = vi.fn(
			async () =>
				new Response(await gated, {
					status: 200,
	...
	it("throws AuthError on token endpoint 4xx", async () => {
		...
		await expect(resolver.resolve(baseAuth, {}, {})).rejects.toBeInstanceOf(
			AuthError,
		);
	});
