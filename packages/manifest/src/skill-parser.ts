...
export function manifestEntryToToolReferences(
	...
	switch (kind) {
		case "mcp":
			return {
				...
				tools: e.tools
					? normalizeMCPToolRefs(e.tools as unknown[], idx)
					: undefined,
			};
		case "local":
			if (!Array.isArray(e.tools)) {
				throw new Error(
					`tools[${idx}].tools must be an array of tool name strings`,
				);
			}
...
function normalizeMCPToolRefs(raw: unknown[], parentIdx: number): MCPToolRef[] {
	return raw.map((item, j) => {
		...
		if (!item || typeof item !== "object") {
			throw new Error(
				`tools[${parentIdx}].tools[${j}] must be a string or { tool: string }`,
			);
		}
