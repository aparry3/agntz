...
export function validateSkill(yaml: string): ValidationResult {
  ...
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    errors.push({ level: "structural", path: "", message: "Skill YAML must be an object" });
    return { valid: false, errors, warnings };
  }
  ...
  if (obj.tools !== undefined) {
    if (!Array.isArray(obj.tools)) {
      errors.push({ level: "structural", path: "tools", message: "tools must be an array" });
    } else {
...
function validateSkillFields(obj: Record<string, unknown>, errors: ValidationError[]): void {
  const name = obj.name;
  if (typeof name !== "string" || name.length === 0) {
    errors.push({ level: "structural", path: "name", message: "name is required and must be a non-empty string" });
  ...
  if (typeof description !== "string" || description.trim().length === 0) {
    errors.push({ level: "structural", path: "description", message: "description is required and must be non-empty" });
  }

  const instructions = obj.instructions;
  if (typeof instructions !== "string" || instructions.trim().length === 0) {
    errors.push({ level: "structural", path: "instructions", message: "instructions is required and must be non-empty" });
  }
}
