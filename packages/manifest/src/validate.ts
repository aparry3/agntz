...
function validateLLMStructural(
	...
	if (!manifest.model) {
		errors.push({
			level: "structural",
			path: p(path, "model"),
			message: "LLM agent must have a model",
		});
	} else {
		...
		if (
			manifest.model.temperature != null &&
			(manifest.model.temperature < 0 || manifest.model.temperature > 2)
		) {
			warnings.push({
				path: p(path, "model.temperature"),
	...
	if (!manifest.instruction || typeof manifest.instruction !== "string") {
		errors.push({
			level: "structural",
			path: p(path, "instruction"),
			message: "LLM agent must have an instruction string",
		});
	} else {
	...
	if (manifest.prompt !== undefined) {
		if (typeof manifest.prompt !== "string") {
			errors.push({
				level: "structural",
				path: p(path, "prompt"),
				message: "LLM agent 'prompt' must be a string when present",
			});
		} else {
	...
	if (manifest.examples) {
		for (let i = 0; i < manifest.examples.length; i++) {
			...
			if (!ex.input || typeof ex.input !== "string") {
				errors.push({
					level: "structural",
					path: p(path, `examples[${i}].input`),
					message: "Example input must be a string",
				});
			}
			if (!ex.output || typeof ex.output !== "string") {
				errors.push({
					level: "structural",
					path: p(path, `examples[${i}].output`),
...
function validateToolStructural(
	...
	if (!manifest.tool) {
		errors.push({
			level: "structural",
			path: p(path, "tool"),
			message: "Tool agent must have a tool configuration",
		});
		return;
	}
	if (
		!manifest.tool.kind ||
		!["mcp", "local", "http"].includes(manifest.tool.kind)
	) {
		errors.push({
			level: "structural",
			path: p(path, "tool.kind"),
			message: "tool.kind must be 'mcp', 'local', or 'http'",
		});
	}
	if (!manifest.tool.name) {
		...
		manifest.tool.kind === "http" &&
		!HTTP_TOOL_NAME_RE.test(manifest.tool.name)
	) {
		errors.push({
			level: "structural",
			path: p(path, "tool.name"),
	...
	if (manifest.tool.kind === "mcp" && !manifest.tool.server) {
		errors.push({
			level: "structural",
			path: p(path, "tool.server"),
			message: "MCP tool must have a server URL",
		});
	}
	...
	if (manifest.tool.params) {
		for (const [key, val] of Object.entries(manifest.tool.params)) {
			if (typeof val !== "string") {
				errors.push({
					level: "structural",
					path: p(path, `tool.params.${key}`),
					message: "Tool param values must be strings (template expressions)",
				});
			} else {
...
function validateSequentialStructural(
	...
	if (!manifest.steps || manifest.steps.length === 0) {
		errors.push({
			level: "structural",
			path: p(path, "steps"),
			message: "Sequential agent must have at least one step",
		});
		return;
	}
	...
	if (manifest.maxIterations != null && !manifest.until) {
		warnings.push({
			path: p(path, "maxIterations"),
			message: "maxIterations has no effect without 'until'",
		});
	}
...
function validateParallelStructural(
	...
	if (!manifest.branches || manifest.branches.length === 0) {
		errors.push({
			level: "structural",
			path: p(path, "branches"),
			message: "Parallel agent must have at least one branch",
		});
		return;
	}
...
function validateStep(
	...
	if (!step.ref && !step.agent) {
		errors.push({
			level: "structural",
			path,
			message: "Step must have either 'ref' or 'agent'",
		});
		return;
	}
	if (step.ref && step.agent) {
	...
	if (step.ref && typeof step.ref !== "string") {
		errors.push({
			level: "structural",
			path: p(path, "ref"),
			message: "ref must be a string (agent ID)",
		});
		...
		for (const [key, val] of Object.entries(step.input)) {
			if (typeof val !== "string") {
				errors.push({
					...
					message: "Input transform values must be template strings",
				});
			} else {
...
function validateSpawnableStructural(
	...
	const spawnable = parent.spawnable ?? [];
	for (let i = 0; i < spawnable.length; i++) {
		...
		if (ref.kind !== "ref" && ref.kind !== "inline") {
			errors.push({
				level: "structural",
				path: epath,
				message: "spawnable entry must have kind 'ref' or 'inline'",
			});
			continue;
		}
		...
		if (ref.kind === "ref") {
			if (!ref.agentId || typeof ref.agentId !== "string") {
				errors.push({
					level: "structural",
					path: p(epath, "agentId"),
					message: "ref entry must have an agentId string",
				});
				continue;
			}
			if (ref.agentId === parent.id) {
				errors.push({
					level: "reference",
					path: p(epath, "agentId"),
					message: `spawnable cannot reference self ('${parent.id}')`,
				});
			}
			if (seenIds.has(ref.agentId)) {
				warnings.push({
					path: p(epath, "agentId"),
					message: `duplicate spawnable agentId '${ref.agentId}'`,
				});
			}
			...
			if (!def) {
				errors.push({
					level: "structural",
					path: p(epath, "definition"),
					message: "inline entry must have a definition",
				});
				continue;
			}
			if (def.id === parent.id) {
				errors.push({
					level: "reference",
					path: p(epath, "definition.id"),
					message: `inline spawnable cannot reuse parent id ('${parent.id}')`,
				});
			}
			...
			if (def.instruction && /\{\{[^}]+\}\}/.test(def.instruction)) {
				errors.push({
					...
					message:
						"spawnable inline child instruction must not contain template variables ({{...}}) — children are pre-registered with static systemPrompts. Use inputSchema + a static prompt that references the spawn input as the user message.",
				});
			}
	...
	for (let i = 0; i < skills.length; i++) {
		...
		if (typeof name !== "string") {
			errors.push({
				level: "structural",
				path: epath,
				message: `skill name must be a string (got ${typeof name})`,
			});
			continue;
		}
		...
		if (entry.kind === "mcp") {
			if (!entry.server) {
				errors.push({
					level: "structural",
					path: p(epath, "server"),
					message: "MCP tool entry must have a server URL",
				});
			}
			if (entry.tools) {
				for (let j = 0; j < entry.tools.length; j++) {
					...
					if (typeof ref === "object") {
						if (!ref.tool) {
							errors.push({
								level: "structural",
								path: `${epath}.tools[${j}].tool`,
								message: "Wrapped tool must have a tool name",
							});
						}
						if (ref.params) {
		...
		if (entry.kind === "local") {
			if (!entry.tools || entry.tools.length === 0) {
				errors.push({
					level: "structural",
					path: epath,
					message: "Local tool entry must list at least one tool",
				});
			} else {
				for (let j = 0; j < entry.tools.length; j++) {
					...
					if (typeof name !== "string") {
						errors.push({
							level: "structural",
							path: `${epath}.tools[${j}]`,
							message: "Local tool name must be a string",
						});
						continue;
					}
		...
		if (entry.kind === "agent") {
			if (!entry.agent) {
				errors.push({
					level: "structural",
					path: p(epath, "agent"),
					message: "Agent tool entry must reference an agent ID",
				});
				...
				if (typeof entry.version !== "string") {
					...
					errors.push({
						...
						path: p(epath, "version"),
						message:
							"agent tool entry must not combine an '@version' suffix in 'agent' with a separate 'version' field",
					});
				} else {
					try {
		...
		if (entry.kind === "http") {
			...
			if (!entry.name || typeof entry.name !== "string") {
				errors.push({
					level: "structural",
					path: p(epath, "name"),
					message: "HTTP tool entry must have a name string",
				});
			...
			if (!entry.url || typeof entry.url !== "string") {
				errors.push({
					level: "structural",
					path: p(epath, "url"),
					message: "HTTP tool entry must have a url string",
				});
			} else {
			...
			const placeholderNames: Set<string> = new Set();
			if (entry.url && typeof entry.url === "string") {
				...
				for (const [key, val] of Object.entries(entry.params)) {
					...
					if (typeof val !== "string") {
						errors.push({
							...
							path: p(epath, `params.${key}`),
							message:
								"HTTP tool param values must be strings (template expressions).",
						});
					} else {
				...
				for (const [key, val] of Object.entries(entry.headers)) {
					if (typeof val !== "string") {
						errors.push({
							...
							path: p(epath, `headers.${key}`),
							message:
								"HTTP tool header values must be strings (template expressions).",
						});
					} else {
						validateTemplatesSyntax(val, p(epath, `headers.${key}`), errors);
					}
	...
	for (const [name, entry] of Object.entries(resources)) {
		...
		if (!entry.kind || typeof entry.kind !== "string") {
			errors.push({
				level: "structural",
				path: p(epath, "kind"),
				message: "Resource kind must be a string",
			});
		...
		if (
			...
			errors.push({
				level: "structural",
				path: p(epath, "mode"),
				message: "Resource mode must be 'read' or 'read-write'",
			});
		}
		if (entry.namespace !== undefined) {
			...
			if (typeof namespace === "string") {
				...
				namespace.every((v) => typeof v === "string")
			) {
				// ok
			} else {
				errors.push({
...
function resourceToolPrefixes(
	resources: Record<string, ResourceManifestEntry> | undefined,
): string[] {
	return Object.keys(resources ?? {}).map(
		(name) => `${resourceToolPrefix(name)}_`,
	);
}
...
function validateOAuth2ClientCredentials(
	...
	if (
		a.creds_location !== undefined &&
		a.creds_location !== "basic_header" &&
		a.creds_location !== "body"
	) {
		errors.push({
			level: "structural",
...
function validateTokenExchange(
	...
	if (!req || typeof req !== "object") {
		errors.push({
			level: "structural",
			path: p(path, "request"),
			message: "auth.request is required",
		});
	} else {
		...
		if (
			req.body_type !== undefined &&
			!["json", "form", "query"].includes(req.body_type as string)
		) {
			errors.push({
				level: "structural",
		...
		if (req.body !== undefined) {
			validateBodyTemplates(
				req.body,
				req.body_type as string | undefined,
				p(path, "request.body"),
				errors,
			);
		}
	...
	if (!ext || typeof ext !== "object") {
		errors.push({
			level: "structural",
			path: p(path, "extract"),
			message: "auth.extract is required",
		});
	} else {
		if (
			ext.response_format !== undefined &&
			ext.response_format !== "json" &&
			ext.response_format !== "text"
		) {
			errors.push({
				level: "structural",
		...
		if (!isText) {
			if (typeof ext.token_path !== "string" || ext.token_path.length === 0) {
				...
				errors.push({
					...
					path: p(path, "extract.token_path"),
					message:
						"auth.extract.token_path must be a JSONPath starting with '$'.",
				});
			}
		}
		if (ext.expires_path !== undefined) {
			if (
				typeof ext.expires_path !== "string" ||
				!ext.expires_path.startsWith("$")
			) {
				errors.push({
					level: "structural",
					path: p(path, "extract.expires_path"),
					message:
						"auth.extract.expires_path must be a JSONPath starting with '$'.",
				});
			}
		}
	}
		...
		if (app.format !== undefined) {
			if (typeof app.format !== "string" || !app.format.includes("{token}")) {
				errors.push({
					...
					message:
						"auth.apply.format must be a string containing the literal '{token}'.",
				});
			}
		}
	}
...
function validateRefreshOn(
	...
	if (
		...
		errors.push({
			...
			message:
				"auth.refresh_on must be an array of HTTP status code integers (100-599).",
		});
	}
}

function validateCacheTtl(
	val: unknown,
	path: string,
	errors: ValidationError[],
): void {
...
function validateTemplatesSyntax(
	...
	if (ifOpens !== ifCloses) {
		errors.push({
			level: "structural",
			path,
			message: `Unbalanced conditional blocks: ${ifOpens} {{#if}} vs ${ifCloses} {{/if}}`,
		});
	}
}
	...
	for (let i = 0; i < manifest.steps.length; i++) {
		...
		if (inputVars.includes(stateKey)) {
			errors.push({
				level: "reference",
				path: stepPath,
				message: `stateKey '${stateKey}' collides with input property`,
			});
		}
		if (stateKeys.has(stateKey)) {
			warnings.push({
				path: stepPath,
				message: `stateKey '${stateKey}' is used by multiple steps — later step overwrites earlier`,
			});
		}
...
async function validateExternal(
	...
	switch (manifest.kind) {
		case "llm":
			...
			if (manifest.spawnable) {
				await validateSpawnableExternal(
					manifest.spawnable,
					p(path, "spawnable"),
					errors,
					ctx,
	...
	if (step.ref) {
		...
		if (!exists) {
			errors.push({
				level: "external",
				path: p(path, "ref"),
				message: `Referenced agent '${step.ref}' not found`,
			});
		}
	}
	if (step.agent) {
			...
			for (const name of entry.tools) {
				if (!ctx.localTools.includes(name)) {
					errors.push({
						level: "external",
						path: epath,
						message: `Local tool '${name}' is not registered`,
					});
				}
			}
		}
		...
		if (entry.kind === "agent") {
			...
			if (!exists) {
				errors.push({
					level: "external",
					path: p(epath, "agent"),
					message: `Referenced agent '${entry.agent}' not found`,
				});
			}
		}
	...
	if (manifest.tool.kind === "local") {
		if (!ctx.localTools.includes(manifest.tool.name)) {
			errors.push({
				level: "external",
				path: p(path, "tool.name"),
				message: `Local tool '${manifest.tool.name}' is not registered`,
			});
		}
	}
}
 ...
 */
function getStaticOutputKeysFromManifest(
