...
function normalizeLLM(
	...
	return {
		...
		spawnable: raw.spawnable
			? normalizeSpawnable(raw.spawnable as unknown[])
			: undefined,
...
function normalizeResources(
	raw: unknown,
): Record<string, ResourceManifestEntry> {
...
function normalizeReply(
	raw: unknown,
): boolean | { maxPerRun?: number } | undefined {
	...
	return raw.map((entry, i) => {
		...
		if (kind === "inline") {
			...
			if (!def || typeof def !== "object") {
				throw new Error(
					`spawnable[${i}].definition is required for inline refs`,
				);
			}
			const inlineManifest = normalizeManifest(def as Record<string, unknown>);
			if (inlineManifest.kind !== "llm") {
				throw new Error(
					`spawnable[${i}].definition must be an llm-kind manifest (got ${inlineManifest.kind})`,
				);
			}
			return { kind: "inline" as const, definition: inlineManifest };
		}
		throw new Error(
			`spawnable[${i}].kind must be 'ref' or 'inline' (got '${kind}')`,
		);
	});
}
...
function normalizeSkills(raw: unknown[]): string[] {
	...
	return raw.map((entry, i) => {
		...
		if (!SKILL_NAME_RE.test(entry)) {
			throw new Error(
				`skills[${i}] '${entry}' must match ${SKILL_NAME_RE.source}`,
			);
		}
		return entry;
	});
...
function normalizeSequential(
	...
	if (!steps || !Array.isArray(steps))
		throw new Error("Sequential agent must have a 'steps' array");
...
function normalizeParallel(
	...
	if (!branches || !Array.isArray(branches))
		throw new Error("Parallel agent must have a 'branches' array");
...
function normalizeStep(raw: unknown): StepRef {
	...
	if (typeof step.ref === "string") {
		...
		throw new Error(
			"Step must have either 'ref' (agent ID) or 'agent' (inline definition)",
		);
	}
...
function normalizeModel(raw: unknown): LLMAgentManifest["model"] {
	if (!raw || typeof raw !== "object")
		throw new Error("LLM agent must have a 'model' field");
		...
		switch (kind) {
			case "mcp":
				return {
					...
					tools: e.tools
						? normalizeMCPToolRefs(e.tools as unknown[])
						: undefined,
