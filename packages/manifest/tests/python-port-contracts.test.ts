...
function readManifest(filename: string) {
	return parseManifest(
		readFileSync(join(CONTRACTS, "manifests", filename), "utf8"),
	);
}
...
describe("python port contract fixtures", () => {
	it("parses shared manifest kind fixtures", () => {
		...
		for (const [filename, expected] of Object.entries(expectations)) {
			...
			expect(
				manifest.stateKey ??
					manifest.id.replace(/-([a-z])/g, (_, c) => c.toUpperCase()),
			...
			if (expected.outputSchema) {
				expect(manifest.kind).toBe("llm");
				if (manifest.kind === "llm")
					expect(manifest.outputSchema).toEqual(expected.outputSchema);
			}

			if (expected.toolKind) {
				expect(manifest.kind).toBe("tool");
				if (manifest.kind === "tool")
					expect(manifest.tool.kind).toBe(expected.toolKind);
			}

			if (expected.stepKeys) {
				expect(manifest.kind).toBe("sequential");
				if (manifest.kind === "sequential") {
					expect(manifest.steps.map((step) => getStateKey(step))).toEqual(
						expected.stepKeys,
					);
				}
			}
			...
			if (expected.branchKeys) {
				...
				if (manifest.kind === "parallel") {
					expect(
						manifest.branches.map((branch) => getStateKey(branch)),
					).toEqual(expected.branchKeys);
				}
			}
		}
	});
	...
	it("matches shared state, template, and output mapping fixtures", () => {
		...
		const state = createInitialState(contract.initialInput, {
			userQuery: "string",
		});

		expect(state).toEqual(contract.initialState);
		expect(
			interpolate(contract.templates.simpleReference.template, state),
		).toBe(contract.templates.simpleReference.value);
		...
		expect(
			applyOutputMapping(
				contract.outputMapping.mapping,
				contract.outputMapping.state,
			),
