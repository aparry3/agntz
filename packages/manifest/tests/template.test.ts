...
describe("resolvePath", () => {
	...
	it("resolves nested properties", () => {
		expect(
			resolvePath({ agent: { output: { score: 0.9 } } }, "agent.output.score"),
		).toBe(0.9);
	});
...
describe("interpolate", () => {
	...
	it("replaces nested variables", () => {
		expect(
			interpolate("Score: {{agent.score}}", { agent: { score: 42 } }),
		).toBe("Score: 42");
	});
...
describe("renderTemplate", () => {
	it("processes conditional blocks with truthiness", () => {
		...
		expect(renderTemplate(template, { feedback: "good" })).toBe(
			"Start Feedback: good End",
		);
