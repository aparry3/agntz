...
`);
    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => e.path === "name" && e.message.includes("must match"))).toBe(true);
  });
...
describe("validateSkillFull - external", () => {
  function mockCtx(overrides?: Partial<SkillValidationContext>): SkillValidationContext {
    return {
...
`,
    ...
    expect(
      result.errors.some(
        (e) => e.level === "external" && e.message.includes("Connection refused"),
      ),
    ).toBe(true);
  });
