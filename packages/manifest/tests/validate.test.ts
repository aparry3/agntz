...
`);
		expect(result.valid).toBe(false);
		expect(result.errors.some((e) => e.message.includes("instruction"))).toBe(
			true,
		);
	});
...
`);
		expect(result.valid).toBe(false);
		expect(result.errors.some((e) => e.message.includes("Unmatched"))).toBe(
			true,
		);
	});
...
`);
		expect(result.valid).toBe(false);
		expect(
			result.errors.some((e) => e.message.includes("Unbalanced conditional")),
		).toBe(true);
	});
...
`);
		expect(result.valid).toBe(false);
		expect(
			result.errors.some((e) => e.path.includes("resources.memory.mode")),
		).toBe(true);
		expect(
			result.errors.some((e) => e.path.includes("resources.memory.namespace")),
		).toBe(true);
	});
...
`);
		...
		expect(
			result.errors.some((e) =>
				e.message.includes("reserved resource tool prefix"),
			),
		).toBe(true);
	});
...
`);
		expect(
			result.warnings.some((w) => w.message.includes("maxIterations")),
		).toBe(true);
	});
...
`);
		...
		expect(
			result.errors.some(
				(e) => e.level === "reference" && e.message.includes("nonExistent"),
			),
		).toBe(true);
	});
...
`);
		...
		expect(
			result.errors.filter(
				(e) =>
					e.level === "reference" &&
					(e.message.includes("validator") || e.message.includes("generator")),
			),
		).toHaveLength(0);
	});
...
`);
		expect(
			result.errors.filter((e) => e.message.includes("agentA")),
		).toHaveLength(0);
	});
...
`);
		expect(
			result.errors.some((e) => e.message.includes("collides with input")),
		).toBe(true);
	});
...
`);
		expect(
			result.warnings.some((w) => w.message.includes("multiple steps")),
		).toBe(true);
	});
...
`);
		expect(
			result.errors.some((e) =>
				e.message.includes("'stranger' is not declared"),
			),
		).toBe(true);
	});
...
`);
		expect(
			result.errors.some((e) => e.message.includes("missing key 'style'")),
		).toBe(true);
	});
...
`);
		expect(
			result.errors.some((e) =>
				e.message.includes("does not provide key 'wrongKey'"),
			),
		).toBe(true);
	});
...
`,
		...
		expect(
			result.errors.some((e) => e.message.includes("nonExistentTool")),
		).toBe(true);
	});
...
`,
		...
		expect(
			result.errors.some((e) => e.path.includes("headers.X-Numeric")),
		).toBe(true);
	});
...
`,
		...
		expect(result.errors.some((e) => e.message.includes("unknown_tool"))).toBe(
			true,
		);
	});
...
`,
		...
		expect(result.warnings.some((w) => w.message.includes("ENOTFOUND"))).toBe(
			true,
		);
		expect(result.valid).toBe(true);
	});
...
`,
		...
		expect(
			result.errors.some(
				(e) => e.level === "external" && e.message.includes("ENOTFOUND"),
			),
		).toBe(true);
		expect(result.valid).toBe(false);
	});
...
`,
		...
		expect(
			result.errors.some((e) => e.message.includes("nonexistent_tool")),
		).toBe(true);
	});
...
`,
		...
		expect(
			result.errors.some(
				(e) => e.level === "external" && e.message.includes("missing-child"),
			),
		).toBe(true);
	});
...
`,
		...
		expect(
			result.errors.filter((e) => e.message.includes("researcher")),
		).toHaveLength(0);
		expect(result.valid).toBe(true);
	});
});
...
`);
		expect(
			result.errors.some(
				(e) =>
					e.level === "reference" &&
					e.message.includes("cannot reference self"),
			),
		).toBe(true);
	});
...
`);
		expect(
			result.warnings.some((w) => w.message.includes("duplicate spawnable")),
		).toBe(true);
	});
...
`);
		...
		expect(
			result.errors.some(
				(e) => e.level === "structural" && /version/i.test(e.message),
			),
		).toBe(true);
	});
});
...
`);
		expect(
			result.errors.some(
				(e) =>
					e.level === "structural" &&
					e.message.includes("Optional placeholders") &&
					e.message.includes("query string"),
			),
		).toBe(true);
	});
...
`);
		expect(
			result.errors.some(
				(e) =>
					e.level === "structural" &&
					e.message.includes("params.foo") &&
					e.message.includes("does not correspond"),
			),
		).toBe(true);
	});
...
`);
		expect(
			result.errors.some(
				(e) =>
					e.level === "structural" &&
					e.message.includes("HTTP tool method must be one of"),
			),
		).toBe(true);
	});
...
`);
		expect(
			result.errors.some(
				(e) => e.level === "structural" && e.message.includes("client_id"),
			),
		).toBe(true);
		expect(
			result.errors.some(
				(e) => e.level === "structural" && e.message.includes("client_secret"),
			),
		).toBe(true);
	});
...
`);
		expect(
			result.errors.some(
				(e) => e.level === "structural" && e.message.includes("token_path"),
			),
		).toBe(true);
	});
...
`);
		expect(
			result.errors.some(
				(e) =>
					e.level === "structural" &&
					e.message.includes("apply.format") &&
...
`);
		expect(
			result.errors.some(
				(e) =>
					e.level === "structural" &&
					e.message.includes("not an allowed outbound URL"),
			),
		).toBe(true);
	});
...
`);
		expect(
			result.errors.some(
				(e) => e.level === "structural" && e.message.includes("get-user"),
			),
		).toBe(true);
	});
});
...
`;
		...
		try {
			...
			expect(
				result.errors.some(
					(e) =>
						e.level === "external" &&
						e.message.includes("Could not reach HTTP endpoint"),
				),
			).toBe(true);
			expect(result.valid).toBe(false);
		} finally {
	...
	it("emits a warning (not error) for an unreachable host when not strict", async () => {
		...
		try {
			...
			expect(
				result.errors.filter(
					(e) =>
						e.level === "external" && e.message.includes("Could not reach"),
				),
			).toHaveLength(0);
			expect(
				result.warnings.some((w) =>
					w.message.includes("Could not reach HTTP endpoint"),
				),
			).toBe(true);
			expect(result.valid).toBe(true);
		} finally {
	...
	it("emits a warning (never an error) when a referenced secret is missing", async () => {
		...
		try {
			...
			expect(
				result.errors.filter((e) => e.message.includes("missing_token")),
			).toHaveLength(0);
			expect(
				result.warnings.some((w) => w.message.includes("missing_token")),
			).toBe(true);
		} finally {
			setFetch(undefined);
		}
	...
	it("does not emit a warning when a referenced secret exists", async () => {
		...
		try {
			...
			expect(
				result.warnings.filter((w) => w.message.includes("missing_token")),
			).toHaveLength(0);
		} finally {
			setFetch(undefined);
		}
...
`;
	...
	it("emits a warning (never an error) when a referenced env var is missing", async () => {
		...
		try {
			...
			expect(
				result.errors.filter((e) => e.message.includes("MY_API_TOKEN")),
			).toHaveLength(0);
			expect(
				result.warnings.some((w) => w.message.includes("MY_API_TOKEN")),
			).toBe(true);
		} finally {
			setFetch(undefined);
		}
	...
	it("does not emit a warning when a referenced env var exists", async () => {
		...
		try {
			...
			expect(
				result.warnings.filter((w) => w.message.includes("MY_API_TOKEN")),
			).toHaveLength(0);
		} finally {
			setFetch(undefined);
		}
