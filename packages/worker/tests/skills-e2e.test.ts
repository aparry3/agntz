...
describe("worker skills e2e (runner + skill redaction)", () => {
  it("after a run completes, the persisted session has the redacted use_skill output", async () => {
    ...
    await userStore.putSkill({
      ...
      instructions: "search broadly and cite. This is sensitive content that must be redacted.",
    ...
    const result = await runner.invoke("researcher-bot", "Tell me about agentic AI.", {
      sessionId,
      userId: "u1",
    });
    ...
    expect(result.toolCalls.map((c) => c.name)).toEqual(["use_skill", "web_search"]);
    ...
    const useSkillCall = assistantMsg!.toolCalls!.find((tc) => tc.name === "use_skill");
    ...
    const webSearchCall = assistantMsg!.toolCalls!.find((tc) => tc.name === "web_search");
