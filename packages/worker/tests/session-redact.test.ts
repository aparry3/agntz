...
function makeToolCall(
	overrides: Partial<ToolCallRecord> & { id: string; name: string },
): ToolCallRecord {
	return {
		id: overrides.id,
		name: overrides.name,
...
describe("redactSkillToolResults", () => {
	...
	it("redacts multiple use_skill calls across multiple messages", () => {
		...
		const second = redacted[2].toolCalls?.[0].output as {
			instructions: string;
		};
	...
	it("does not mutate the input messages", () => {
		...
		expect(
			(messages[0].toolCalls?.[0].output as { instructions: string })
				.instructions,
		).toBe("real content");
	});
});
