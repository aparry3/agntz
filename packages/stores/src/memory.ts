...
import { decryptSecret, encryptSecret, getLastFour } from "@agntz/contracts";
import type { AgntzStore, ApiKeyRecord, WebhookDelivery } from "./contracts.js";
...
export interface HostedMemoryBackend {
...
function createHostedMemoryBackend(): HostedMemoryBackend {
	return {
		apiKeys: new Map(),
...
export class MemoryStore implements AgntzStore {
	protected backend: MemoryBackend;
	private hostedBackend: HostedMemoryBackend;
	...
	constructor(
		opts: {
			...
			hostedBackend?: HostedMemoryBackend;
			strict?: boolean;
		} = {},
	) {
		this.backend = opts.backend ?? createMemoryBackend();
		this.hostedBackend = opts.hostedBackend ?? createHostedMemoryBackend();
	...
	forUser(userId: string): MemoryStore {
		return new MemoryStore({
			...
			hostedBackend: this.hostedBackend,
		});
	}
	...
	private requireHostedUser(): string {
		if (!this.userId) {
			throw new Error("MemoryStore: user not set. Call forUser(id) first.");
		}
		return this.userId;
	...
	async createApiKey(params: { userId: string; name: string }): Promise<{
		...
		this.hostedBackend.apiKeys.set(row.id, row);
		this.hostedBackend.apiKeyByHash.set(keyHash, row);
		return { record: apiKeyRowToRecord(row), rawKey };
	}

	async listApiKeys(userId: string): Promise<ApiKeyRecord[]> {
		return Array.from(this.hostedBackend.apiKeys.values())
			.filter((r) => r.userId === userId)
			.map(apiKeyRowToRecord)
			.sort((a, b) => b.createdAt.localeCompare(a.createdAt));
	}

	async revokeApiKey(params: { userId: string; keyId: string }): Promise<void> {
		const row = this.hostedBackend.apiKeys.get(params.keyId);
	...
	async resolveApiKey(
		...
		const keyHash = createHash("sha256").update(rawKey).digest("hex");
		const row = this.hostedBackend.apiKeyByHash.get(keyHash);
	...
	async listNamespaceRoots(userId: string): Promise<string[]> {
		return Array.from(
			this.hostedBackend.namespaceRoots.get(userId) ?? [],
		).sort();
	}

	async addNamespaceRoot(userId: string, root: string): Promise<void> {
		const normalized = normalizeNamespaceGrant(root);
		let roots = this.hostedBackend.namespaceRoots.get(userId);
		if (!roots) {
			roots = new Set();
			this.hostedBackend.namespaceRoots.set(userId, roots);
		}
	...
	async removeNamespaceRoot(userId: string, root: string): Promise<void> {
		const normalized = normalizeNamespaceGrant(root);
		this.hostedBackend.namespaceRoots.get(userId)?.delete(normalized);
	}
	...
	async insert(
		...
		this.hostedBackend.webhookDeliveries.set(delivery.id, row);
		return delivery.id;
	}
	...
	async updateStatus(
		...
		const row = this.hostedBackend.webhookDeliveries.get(id);
		if (!row) return;
		row.status = status;
	...
	> {
		...
		for (const r of this.hostedBackend.webhookDeliveries.values()) {
