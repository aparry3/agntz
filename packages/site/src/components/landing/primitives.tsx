...
export function Btn({
	...
	const pad =
		size === "lg" ? "12px 20px" : size === "sm" ? "6px 12px" : "10px 16px";
	...
	const base: CSSProperties = {
		...
		whiteSpace: "nowrap",
		boxShadow: primary
			? "0 1px 0 rgba(26,25,22,0.06), 0 2px 4px rgba(26,25,22,0.08)"
			: "none",
		...style,
	};
...
export function Section({
	...
	return (
		...
		>
			<div
				style={{ width: "min(1180px, calc(100% - 64px))", margin: "0 auto" }}
			>
				{kicker && (
					<div
						style={{
...
export function Stack({
	...
	return (
		<div style={{ display: "flex", flexDirection: dir, gap, ...style }}>
			{children}
		</div>
	);
}
...
export function Row({
	...
	return (
		<div
			onClick={onClick}
			style={{ display: "flex", flexDirection: "row", gap, ...style }}
		>
			{children}
		</div>
	);
