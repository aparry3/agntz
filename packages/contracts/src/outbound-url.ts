...
export class OutboundUrlPolicyError extends Error {
	...
	constructor(
		message: string,
		opts?: { code?: string; url?: string; cause?: unknown },
	) {
		super(message);
...
export function validateOutboundUrl(
	...
	if (
		protocol !== "https:" &&
		!(options.allowHttp !== false && protocol === "http:")
	) {
...
export async function fetchWithOutboundPolicy(
	...
	for (let redirects = 0; redirects <= maxRedirects; redirects++) {
		...
		if (redirects === maxRedirects) {
			throw new OutboundUrlPolicyError(
				`Too many redirects; max is ${maxRedirects}`,
				{
...
async function resolveHostname(
	...
	try {
		...
		throw new OutboundUrlPolicyError(
			`Hostname resolution failed: ${hostname}`,
			{
