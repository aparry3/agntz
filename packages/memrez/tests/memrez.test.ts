...
import { createMemrez, MemrezScopeError } from "../src/index.js";
import type { CurateOp, MemrezReasoner, TaggerInput, TaggerResult } from "../src/index.js";
...
describe("memrez core", () => {
  ...
  it("reads ancestor scopes but not siblings", async () => {
    ...
    expect(prefs.map((entry) => entry.content)).toEqual(["User 123 preference."]);
  });
  ...
  it("allows descendant writes by default and rejects siblings or ancestors", async () => {
    ...
    await expect(
      memrez.write(["app/user/u_123"], "scope:app/user|topic:prefs|Bad ancestor."),
    ).rejects.toBeInstanceOf(MemrezScopeError);
  });
  ...
  it("dedupes exact active entries in the same scope", async () => {
    ...
    const first = await memrez.write(["app/user/u_123"], "topic:prefs|Prefers metric.");
    const second = await memrez.write(["app/user/u_123"], "topic:prefs|Prefers metric.");
  ...
  it("applies curator supersede and topic blurb operations", async () => {
    ...
    const oldA = await memrez.write(["app/user/u_123"], "topic:prefs|Likes SMS.");
    const oldB = await memrez.write(["app/user/u_123"], "topic:prefs|Prefers email.");
    ...
    expect(entries.map((entry) => entry.content)).toEqual(["Prefers email over SMS."]);
...
function parseDirective(raw: string): {
  ...
  const out: ReturnType<typeof parseDirective> = { content: parts.at(-1)?.trim() ?? raw };
  for (const part of parts.slice(0, -1)) {
    const [key, value] = part.split(":");
    if (key === "scope") out.namespace = value;
    if (key === "topic") out.topics = value.split(",").map((topic) => topic.trim());
