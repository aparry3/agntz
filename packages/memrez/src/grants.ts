...
export function normalizeWritePolicy(
	...
	return {
		...
		ancestorPromotion:
			policy?.ancestorPromotion ?? DEFAULT_WRITE_POLICY.ancestorPromotion,
	};
}
...
export function normalizeGrants(
	...
	if (normalized.length === 0) {
		throw new MemrezScopeError(
			"memrez operations require at least one namespace grant",
		);
	}
	return normalized;
}
...
export function visibleScopes(
	...
	for (const grant of grants) {
		const scopes = includeAncestors
			? namespaceAncestors(grant)
			: [normalizeNamespaceGrant(grant)];
...
export function assertReadableScope(
	...
	if (!readable) {
		throw new MemrezScopeError(
			`scope '${target}' is not readable from grants [${grants.join(", ")}]`,
		);
	}
...
export function assertWritableScope(
	...
	for (const grant of grants) {
		...
		if (
			policy.descendants &&
			isSameOrDescendantNamespace(normalizedTarget, grant)
		) {
			return normalizedTarget;
		}
		if (
			isAllowedAncestorPromotion(
				grant,
				normalizedTarget,
				policy.ancestorPromotion,
			)
		) {
			return normalizedTarget;
		}
