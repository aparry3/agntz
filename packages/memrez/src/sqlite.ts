...
export class SqliteMemoryStore implements MemoryStore {
  ...
  async putEntry(entry: MemoryEntry): Promise<void> {
    ...
    const tx = this.db.transaction(() => {
      ...
      this.db.prepare("DELETE FROM memrez_entry_topics WHERE entry_id = ?").run(entry.id);
  ...
  async setTopicMeta(
    ...
    ).run(scope, topic, meta.blurb ?? null, meta.lastUpdatedAt ?? new Date().toISOString());
  }
  ...
  private getSchemaVersion(): number {
    try {
      const row = this.db
        .prepare("SELECT version FROM memrez_schema_version ORDER BY version DESC LIMIT 1")
  ...
  private findTopicMeta(scopePaths: string[], topic: string): { blurb?: string; lastUpdatedAt: string } | null {
    const stmt = this.db.prepare(
    ...
    for (let i = scopePaths.length - 1; i >= 0; i--) {
      const row = stmt.get(scopePaths[i], topic) as Omit<TopicMetaRow, "topic"> | undefined;
      if (row) {
        return {
          blurb: row.blurb ?? undefined,
          lastUpdatedAt: row.last_updated_at,
  ...
  private rowToEntry(row: EntryRow): MemoryEntry {
    const topics = this.db
      .prepare("SELECT topic FROM memrez_entry_topics WHERE entry_id = ? ORDER BY topic ASC")
