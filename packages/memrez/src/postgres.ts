...
export class PostgresMemoryStore implements MemoryStore {
	...
	async putEntry(entry: MemoryEntry): Promise<void> {
        ...
        ON CONFLICT(id) DO UPDATE SET
          ...
          updated_at = excluded.updated_at`,
			...
			await client.query(
				`DELETE FROM ${this.table("entry_topics")} WHERE entry_id = $1`,
				[entry.id],
			);
	...
	private async findTopicMeta(
		scopePaths: string[],
		topic: string,
	): Promise<TopicMetaRow | null> {
...
function normalizeTablePrefix(prefix: string | undefined): string {
	...
	if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(prefix)) {
		throw new Error(
			"PostgresMemoryStore tablePrefix must be a valid identifier prefix",
		);
	}
	return prefix;
}
