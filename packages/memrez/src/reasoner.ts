...
export interface AgntzClientLike {
  agents: {
    run(input: { agentId: string; input?: unknown; context?: string[]; sessionId?: string }): Promise<AgntzRunResult>;
  };
}
  ...
  return {
    ...
    async curate(input: CuratorInput): Promise<CurateOp[]> {
      ...
      if (!parsed || typeof parsed !== "object" || !Array.isArray((parsed as { ops?: unknown }).ops)) {
        throw new Error("memrez curator returned invalid output: expected { ops: [] }");
      }
...
function parseTaggerResult(result: AgntzRunResult): TaggerResult {
  ...
  return {
    ...
    duplicateOf: typeof obj.duplicateOf === "string" ? obj.duplicateOf : undefined,
  } as TaggerResult;
}
