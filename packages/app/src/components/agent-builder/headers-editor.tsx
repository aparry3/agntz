 ...
 */
export function HeadersEditor({ headers, onChange, secrets }: HeadersEditorProps) {
  ...
  const addBearer = (secretName: string) => {
    ...
    const idx = rows.findIndex((r) => r.key.trim().toLowerCase() === "authorization");
    ...
    <div className="grid gap-2 sm:grid-cols-[minmax(0,220px)_minmax(0,1fr)_auto]">
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex justify-end">
          <InsertSecretMenu
            secrets={secrets}
            onPick={(name) => onChange({ value: appendSecretRef(row.value, name) })}
          />
        </div>
      </div>
