...
export function PipelineCanvas({ pipeline, selectedId, onSelect, onAddStep }: PipelineCanvasProps) {
  return (
    <div
      style={{
...
function PipelineNodeView({ node, selectedId, onSelect, onAddStep }: NodeViewProps) {
  const isContainer = node.kind === "sequential" || node.kind === "parallel";
  if (!isContainer) {
    return <Block node={node} selected={selectedId === node.id} onSelect={onSelect} />;
  }
  ...
  return (
    ...
    >
      ...
      {node.isLoop && node.loop && <LoopFooter loop={node.loop} kind={node.kind} />}
    </div>
  );
}
...
function ContainerHeader({ node }: { node: PipelineNode }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontFamily: FONT_MONO, fontSize: 10, color: NEUTRAL.textSubtle }}>{node.id}</span>
      {node.stateKey && (
        <span
          style={{
            display: "inline-flex",
  ...
  return (
    ...
    >
      ...
      {loop.maxIterations != null && (
        <span style={{ fontFamily: FONT_MONO, fontSize: 10, color: NEUTRAL.textSubtle }}>
...
function KindBadge({ kind }: { kind: PipelineKind }) {
  ...
  return (
    ...
    >
      <span style={{ width: 6, height: 6, borderRadius: 6, background: c.dot }} />
      {c.label}
    </span>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        <span style={{ fontFamily: FONT_MONO, fontSize: 11, fontWeight: 600, color: NEUTRAL.ink }}>
          {node.model?.name || "(no model)"}
        </span>
        {node.model?.provider && (
          <span style={{ fontFamily: FONT_MONO, fontSize: 10.5, color: NEUTRAL.textSubtle }}>
            · {node.model.provider}
          </span>
        )}
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          {Object.entries(node.toolParams).map(([k, v]) => (
            ...
            >
              ...
              <span style={{ color: NEUTRAL.ink, overflow: "hidden", textOverflow: "ellipsis" }}>
                {String(v)}
              </span>
            </div>
          ))}
          ...
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {inputKeys.slice(0, 6).map((k) => (
              ...
              >
                ...
                <span style={{ color: NEUTRAL.textMuted, maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis" }}>
                  {inputs[k].replace(/[{}]/g, "")}
                </span>
              </span>
            ))}
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "flex-start" }}>
          ...
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap", alignItems: "center" }}>
            ...
            {outputs.slice(0, 3).map((f) => (
              <span
                key={f.key}
                style={{ fontFamily: FONT_MONO, fontSize: 10, color: NEUTRAL.textSubtle }}
              >
                ·{f.key}
...
function Arrow() {
  return (
    <svg width="14" height="22" viewBox="0 0 14 22" style={{ display: "block", margin: "0 auto" }}>
      <line x1="7" y1="0" x2="7" y2="16" stroke={NEUTRAL.borderStrong} strokeWidth="1.25" />
      <path
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {hasSchema ? (
          (schema ?? []).map((p) => (
            ...
            >
              <span style={{ color: NEUTRAL.ink, fontWeight: 600 }}>{p.key}</span>
              <span style={{ color: NEUTRAL.textSubtle }}>:{p.type}</span>
              {p.default !== undefined && (
                <span style={{ color: NEUTRAL.textSubtle }}>= {String(p.default)}</span>
              )}
            </span>
          ))
        ) : (
          <span style={{ fontFamily: FONT_SANS, fontSize: 11.5, color: NEUTRAL.textSubtle }}>
            {empty ?? "—"}
          </span>
        )}
      </div>
