...
function Header({
	node,
	onRemove,
}: { node: PipelineNode; onRemove?: () => void }) {
	...
	return (
		...
		>
			<div style={{ display: "flex", alignItems: "center", gap: 8 }}>
				...
				>
					<span
						style={{ width: 7, height: 7, borderRadius: 7, background: c.dot }}
					/>
					{c.label}
				</span>
				{node.isLoop && (
...
function InspectorBody(props: InspectorProps & { snapshot: StateSnapshot }) {
	...
	return (
		<>
			...
			<AvailableStateSection
				snapshot={props.snapshot}
				highlight={highlightFromInput(selected)}
			/>
...
function AvailableStateSection({
	...
	return (
		<Section
			title={`AVAILABLE STATE · ${snapshot.scope}`}
			collapsible
			defaultExpanded
		>
			<div
			...
			>
				{snapshot.keys.length === 0 ? (
					...
					snapshot.keys.map((k) => (
						<StateRow key={k.key} k={k} depth={0} highlight={highlight} />
					))
				)}
			</div>
			<p
			...
			>
				State is scoped to this pipeline. Sub-agents can&apos;t see parent
				state. Only the keys above can be referenced as{" "}
...
function StateRow({
	...
	return (
		<div style={{ paddingLeft: depth * 12 }}>
			...
			>
				<span style={{ color: NEUTRAL.borderStrong }}>
					{isObject ? "▾" : "·"}
				</span>
				...
				<span
					style={{
						marginLeft: "auto",
						color:
							k.source === "input" ? NEUTRAL.textSubtle : NEUTRAL.textMuted,
						fontSize: 9.5,
					}}
				>
...
function RootIdentitySection({
	...
	const kind: AgentKindOption | "" = (
		AGENT_KINDS as readonly string[]
	).includes(kindValue)
...
function InputSchemaSection({ parsedManifest, setField }: InspectorProps) {
	const raw = isRecord(parsedManifest.inputSchema)
		? parsedManifest.inputSchema
		: {};
	const fields = Object.entries(raw).map(([key, def]) => {
		if (typeof def === "string")
			return { key, type: def, default: undefined as unknown };
		if (isRecord(def))
			return {
				key,
				type: asString(def.type) ?? "string",
				default: def.default,
			};
	...
	const writeAll = (
		next: Array<{ key: string; type: string; default: unknown }>,
	) => {
		if (next.length === 0) {
...
function InputMappingSection({ selected, setField }: InspectorProps) {
	...
	const knownKeys =
		inputs.length > 0 ? inputs.map((f) => f.key) : Object.keys(inputMap);

	return (
		<Section title="INPUT MAPPING">
			{knownKeys.length === 0 ? (
				<EmptyHint>
					No declared inputs. Pin keys here to expose state variables to the
					child.
					...
					{knownKeys.map((key) => {
						...
						return (
							...
							>
								...
								<span
									style={{ textAlign: "center", color: NEUTRAL.borderStrong }}
								>
									←
								</span>
								<TextInput
...
function ModelSection({
	...
	const writeModel = (patch: Record<string, unknown>) => {
		...
		const tempVal =
			patch.temperature !== undefined ? patch.temperature : temperature;
		const tokenVal =
			patch.maxTokens !== undefined ? patch.maxTokens : maxTokens;
	...
	return (
		<Section title="MODEL">
			<div style={{ display: "grid", gap: 8 }}>
				<div
					style={{ display: "grid", gap: 8, gridTemplateColumns: "1fr 1.4fr" }}
				>
					<div>
...
function InstructionSection({
	...
	const agent =
		(getAtPath(parsedManifest, selected.agentPath) as Record<
			string,
			unknown
		>) ?? {};
	...
	return (
		<Section title="INSTRUCTION">
			<TextArea
				value={instruction}
				onChange={(v) =>
					setField([...selected.agentPath, "instruction"], v || undefined)
				}
	...
	return (
		<Section title="PROMPT TEMPLATE">
			<TextArea
				value={prompt}
				onChange={(v) =>
					setField([...selected.agentPath, "prompt"], v || undefined)
				}
		...
		const tools = Array.isArray(value.tools)
			? value.tools
					.map((t) =>
						typeof t === "string"
							? t
							: isRecord(t) && typeof t.tool === "string"
								? t.tool
								: null,
					)
...
function ToolEntryBlock({
	...
	return (
		...
		>
			...
			{entry.kind === "http" && (
				<HttpToolEditor entry={entry} catalog={catalog} onChange={onChange} />
			)}
	...
	return (
		<div style={{ display: "grid", gap: 8 }}>
			...
			<div>
				...
				{optionalInPath && (
					<p style={{ margin: "4px 2px 0", fontSize: 11, color: "#dc2626" }}>
						Optional placeholders ({"{X?}"}) are only allowed in the query
						string.
					</p>
				)}
			</div>
	...
	return (
		<Section
			...
			action={
				<SmallButton
					label="+ Add"
					onClick={() => writeAll([...drafts, { kind: "ref" }])}
				/>
			}
		>
			{drafts.length === 0 ? (
	...
	return (
		<Section title="SKILLS" collapsible defaultExpanded={raw.length > 0}>
			<p style={{ margin: "0 0 8px", fontSize: 11, color: NEUTRAL.textMuted }}>
				Skill names this agent can load mid-run via <code>use_skill</code>.
				Names must match{" "}
...
function ReplySection({ selected, parsedManifest, setField }: InspectorProps) {
	...
	const maxPerRun =
		isRecord(reply) && typeof reply.maxPerRun === "number"
			? reply.maxPerRun
			: undefined;
	...
	return (
		<Section title="REPLY" collapsible defaultExpanded={enabled}>
			<p style={{ margin: "0 0 8px", fontSize: 11, color: NEUTRAL.textMuted }}>
				Register a per-invocation <code>reply</code> tool the model can call to
				deliver intermediate assistant messages mid-run.
			</p>
			<label
				style={{
					display: "flex",
...
function ToolCallSection({
	...
	const writeTool = (patch: Record<string, unknown>) => {
		...
		if (
			paramsVal &&
			Object.keys(paramsVal as Record<string, string>).length > 0
		) {
				...
				<div style={{ display: "grid", gap: 8 }}>
					<div>
						...
						<TextInput
							value={until}
							onChange={(v) =>
								setField([...selected.agentPath, "until"], v || undefined)
							}
					...
					<div>
						...
						<NumberField
							value={maxIterations}
							onChange={(v) =>
								setField([...selected.agentPath, "maxIterations"], v)
							}
							min={1}
							step={1}
							placeholder="3"
...
function OutputMappingSection({ parsedManifest, setField }: InspectorProps) {
	...
	return (
		...
		>
			{rows.length === 0 ? (
				<EmptyHint>
					No output mapping — pipeline returns the final state map.
				</EmptyHint>
			) : (
				<div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
					{rows.map((row, idx) => (
