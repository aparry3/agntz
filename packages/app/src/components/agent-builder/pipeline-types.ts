...
export function setIn(
	...
	let cursor: Record<string | number, unknown> = out as Record<
		string | number,
		unknown
	>;
...
const VALID_KINDS: ReadonlyArray<PipelineKind> = [
	"llm",
	"tool",
	"sequential",
	"parallel",
...
function readOutputSchema(value: unknown): PipelineNode["outputSchemaKeys"] {
	...
	for (const [key, def] of Object.entries(value)) {
		...
		else if (isRecord(def) && typeof def.type === "string")
			out.push({ key, type: def.type });
	}
	return out.length > 0 ? out : undefined;
}
...
export function nodeFromAgent(
	...
	const kind: PipelineKind = (VALID_KINDS as readonly string[]).includes(
		kindRaw ?? "",
	)
	...
	const isLoop =
		kind === "sequential" && (until != null || maxIterations != null);
...
export function computeAvailableStateAt(
	root: PipelineNode,
	targetId: string,
): StateSnapshot {
	const keys: StateRef[] = [];
