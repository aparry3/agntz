...
export function ToolCallRow({ toolCall }: { toolCall: RunToolCall }) {
	...
	return (
		<div
			className={`rounded-2xl border ${errored ? "border-rose-200 bg-rose-50" : "border-stone-200 bg-white"}`}
		>
			<button
				type="button"
				onClick={() => setOpen((o) => !o)}
			...
			>
				...
				<span
					className={`font-mono ${errored ? "text-rose-800" : "text-zinc-800"}`}
				>
					{toolCall.name}
				</span>
			...
			{open && (
				<div className="border-t border-stone-200 px-4 py-3 text-xs">
					...
					{errored && (
						<div>
							<div className="mb-1 uppercase tracking-wider text-rose-700">
								Error
							</div>
