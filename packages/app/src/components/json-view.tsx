...
export function JsonView({
	data,
	depth = 0,
}: { data: unknown; depth?: number }) {
	if (data === null) return <Literal text="null" color={ag.muted} />;
	if (data === undefined) return <Literal text="undefined" color={ag.muted} />;
	if (typeof data === "boolean")
		return <Literal text={String(data)} color={ag.warn} />;
	if (typeof data === "number")
		return <Literal text={String(data)} color={ag.ok} />;
	if (typeof data === "string")
		return <Literal text={`"${data}"`} color={ag.blue} />;
