...
const buildLinks: NavLink[] = [
	...
	},
	{
		...
		label: "Skills",
		Ic: I.Skills,
		matches: (p) => p === "/skills" || p.startsWith("/skills/"),
	},
];
...
const observeLinks: NavLink[] = [
	{
		...
		label: "Runs",
		Ic: I.Runs,
		matches: (p) => p === "/runs" || p.startsWith("/runs/"),
	},
	{
		href: "/traces",
		label: "Traces",
		Ic: I.Traces,
		matches: (p) => p === "/traces" || p.startsWith("/traces/"),
	},
...
export function AppSidebar() {
	...
	return (
		...
		>
			...
			<nav
				style={{
					padding: collapsed ? "4px 8px" : "6px 8px",
					flex: 1,
					overflowY: "auto",
				}}
			>
					...
					{!collapsed ? (
						...
						>
							<I.ChevR
								size={9}
								style={{
									transform: configOpen ? "rotate(90deg)" : "none",
				...
				<div style={{ padding: collapsed ? "4px 8px 6px" : "4px 8px 8px" }}>
					<Link
						...
						style={{
							...
							background: pathname.startsWith("/system")
								? ag.ink
								: "transparent",
							fontSize: 12.5,
							textDecoration: "none",
