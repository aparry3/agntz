...
import { EditableText, EditableNumber, EditableSelect, EditableToggle } from "@/components/v3/editor/editable-fields";
...
function getInputSchema(manifest: Record<string, unknown>): InputSchema | undefined {
...
function getExamples(manifest: Record<string, unknown>): Example[] {
  ...
  return raw.filter(
    (e): e is Example =>
      e !== null && typeof e === "object" && typeof (e as Example).input === "string"
  );
}
...
function SchemaField({
  ...
  name: string;
  def: { type: string; default?: unknown; enum?: unknown[]; min?: number; max?: number };
  value: unknown;
  onChange: (next: unknown) => void;
}) {
  ...
  if (def.enum && def.enum.length > 0) {
    const current = value !== undefined && value !== null ? String(value) : String(def.enum[0]);
