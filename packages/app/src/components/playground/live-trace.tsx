...
function makePlaceholderSummary(traceId: string, agentId: string): TraceSummary {
  return {
    traceId,
    ownerId: "",
...
export function LiveTrace({
  ...
  const [summary, setSummary] = useState<TraceSummary>(() => makePlaceholderSummary(traceId, agentId));
  ...
  useEffect(() => {
    ...
    es.addEventListener("span-end", (e) => {
      try {
        ...
        setSpans((prev) =>
          prev.map((s) => (s.spanId === payload.spanId ? { ...s, ...payload.patch } : s))
        );
      } catch {
        /* skip malformed */
      }
  ...
  const rootSpan = useMemo(() => spans.find((s) => s.parentId === null) ?? spans[0] ?? null, [spans]);
