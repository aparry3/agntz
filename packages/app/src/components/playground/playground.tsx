...
export function Playground({
  ...
  const sessionOptions = useMemo(() => {
    const base: ReadonlyArray<readonly [string, string]> = [
      ...
      ...sessions.map(
        (s) =>
          [s.sessionId, `${s.sessionId.slice(0, 16)}… · ${s.messageCount} msgs`] as const
      ),
    ];
    return base;
  }, [sessions]);
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        ...
        throw new Error(
          (errBody as { error?: string }).error ?? `Run failed: ${res.status} ${res.statusText}`
        );
      }
...
function ResultPane({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <div
        style={{
