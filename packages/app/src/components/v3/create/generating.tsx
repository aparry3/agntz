...
export function CreateGenerating({
  ...
  const stepIndex = Math.max(0, steps.findIndex((s) => s.active));
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", height: "100vh" }}>
      ...
      >
        <GenGraph />
        <GenInspector description={description} steps={steps} elapsedSeconds={elapsedSeconds} etaSeconds={etaSeconds} />
      </div>
    </div>
  );
}
    ...
    >
      ...
      <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", padding: 24 }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <Edge />
          {/* Built */}
          <NodeFinalised name="Planner" tag="LLM" tagBg={ag.blueBg} tagFg={ag.blue}>
            ...
            >
              &quot;Read the user&apos;s goals and decide whether they need a workout, a meal plan, or both.&quot;
            </div>
          </NodeFinalised>
          <Edge />
          {/* Streaming */}
          <NodeStreaming name="Workout Generator" tag="Pipeline" tagBg={ag.purpleBg} tagFg={ag.purple}>
      ...
      >
        ...
        <div style={{ flex: 1 }} />
        <Mono size={11}>You can stop anytime and keep what&apos;s been generated.</Mono>
      </div>
    </div>
  );
}
...
function GenInspector({
  ...
  return (
    <aside style={{ background: ag.surface, overflow: "auto", borderLeft: `1px solid ${ag.line2}` }}>
      <div
        style={{
          padding: "12px 18px",
          borderBottom: `1px solid ${ag.line2}`,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
          {steps.map((s, i) => (
            <div key={i} style={{ display: "flex", gap: 10, paddingBottom: 12, position: "relative" }}>
              ...
              >
                {s.done ? <I.Check size={8} /> : s.active ? <Spinner size={8} /> : null}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {s.sub && (
                  <Mono size={10.5} color={ag.muted} style={{ marginTop: 2, display: "inline-block" }}>
                    {s.sub}
                  </Mono>
                )}
              </div>
