			...
			<div style={{ flex: 1, overflowY: "auto" }}>
				{providers.map((provider) => (
					<ProviderGroup
						...
						selected={value}
						onPick={(modelName) =>
							onPick({ provider: provider.id, name: modelName })
						}
					/>
				))}
			</div>
...
function ProviderGroup({
	...
	const staticModels: ProviderModelEntry[] = provider.models.map((id) => ({
		id,
	}));
	...
	const filtered = search
		? modelsForGroup.filter((m) => {
				const hay =
					`${m.id} ${m.displayName ?? ""} ${provider.name}`.toLowerCase();
	...
	return (
		<div style={{ padding: "6px 4px", borderBottom: `1px solid ${ag.line2}` }}>
			...
			{!provider.configured ? (
				...
				filtered.map((model) => {
					const isSelected =
						selected.provider === provider.id && selected.name === model.id;
					return (
						<button
							key={model.id}
							type="button"
