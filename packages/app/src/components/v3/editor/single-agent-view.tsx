...
import { Btn, Mono, Spinner, Tag, VarHl, ag } from "@/components/v3/primitives";
import { Edge, NodeIO } from "@/components/v3/primitives";
...
import { type ToolEntry, ToolsEditor } from "./tools-editor";
			...
			>
				...
				{view !== "yaml" && rightPaneOverride ? (
					...
					<InstructionPanel
						...
						onChangeInstruction={
							onChange
								? (v) => onChange({ ...manifest, instruction: v || undefined })
								: undefined
						}
						onChangePrompt={
							onChange
								? (v) => onChange({ ...manifest, prompt: v || undefined })
								: undefined
						}
					/>
				) : view !== "yaml" ? (
...
function SingleAgentInspector({
	...
	const patch = (next: Partial<SingleAgentManifest>) =>
		onChange?.({ ...manifest, ...next });
	const patchModel = (
		next: Partial<NonNullable<SingleAgentManifest["model"]>>,
	) => patch({ model: stripUndefined({ ...(manifest.model ?? {}), ...next }) });

	const handleInstruction = onChange
		? (next: string) => patch({ instruction: next })
		: undefined;
	const replyEnabled =
		manifest.reply === true ||
		(typeof manifest.reply === "object" && manifest.reply !== null);
	...
	const brokenInstructionRefs = findBrokenRefs(
		manifest.instruction ?? "",
		inScope,
	);
	const brokenPromptRefs = findBrokenRefs(manifest.prompt ?? "", inScope);
	const allBrokenRefs = Array.from(
		new Set([...brokenInstructionRefs, ...brokenPromptRefs]),
	);
		...
		>
			...
			<div style={{ flex: 1, overflow: "auto" }}>
					...
					>
						...
						<div style={{ minWidth: 0, flex: 1 }}>
							...
							<div style={{ marginTop: 4, color: ag.text2 }}>
								These names aren&apos;t declared as inputs. Add them to the
								schema below or remove the references. (Saving still works —
								runtime templates will leave them empty.)
							</div>
						</div>
					</div>
				)}
				...
				<div style={{ borderTop: `1px solid ${ag.line2}` }}>
					...
					<InsSection
						title="Tools"
						badge={`${manifest.tools?.length ?? 0} attached`}
					>
						<ToolsEditor
							tools={(manifest.tools ?? []) as ToolEntry[]}
							onChange={(next) =>
								patch({
									tools: next as Array<Record<string, unknown>> | undefined,
								})
							}
					...
					<InsSection
						title="Output schema"
						badge={`${outputs.length} field${outputs.length === 1 ? "" : "s"}`}
					>
						<SchemaEditor
							kind="output"
			...
			<FooterHint>
				Templates &amp; tool params can only reference the {inputs.length} input
...
function formatModel(model: SingleAgentManifest["model"]): string {
	...
	const temp =
		typeof model.temperature === "number" ? ` · temp ${model.temperature}` : "";
	const max =
		typeof model.maxTokens === "number" ? ` · max ${model.maxTokens}` : "";
