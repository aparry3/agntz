...
function PickerHeader({
	...
	return (
		...
		>
			...
			<div style={{ fontSize: 12.5, color: ag.ink, fontWeight: 500, flex: 1 }}>
				{title}
			</div>
			<button
				type="button"
...
function KindGrid({ onPick }: { onPick: (kind: AddStepKind) => void }) {
	const options: Array<{ kind: AddStepKind; label: string; sub: string }> = [
		{
			kind: "llm",
			label: "LLM step (inline)",
			sub: "A model call defined right here in the pipeline.",
		},
		{
			kind: "tool",
			label: "Tool step (inline)",
			sub: "Call a single tool with templated params.",
		},
		{
			kind: "sequential",
			label: "Sequential sub-pipeline",
			sub: "Nest a sequence of steps inside this one.",
		},
		{
			kind: "parallel",
			label: "Parallel sub-pipeline",
			sub: "Run several branches concurrently.",
		},
		{
			kind: "ref",
			label: "Reference existing agent",
			sub: "Call a saved agent as a step.",
		},
	];
	return (
		<div style={{ padding: 6 }}>
			{options.map((opt) => (
				...
				>
					<div style={{ fontSize: 13, color: ag.ink, fontWeight: 500 }}>
						{opt.label}
					</div>
