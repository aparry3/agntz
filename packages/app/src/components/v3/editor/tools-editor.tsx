	...
	return (
		...
		>
			...
			>
				...
				>
					{expanded ? (
						<I.Chev size={10} style={{ transform: "rotate(180deg)" }} />
					) : (
						<I.Sliders size={11} />
					)}
				</IconBtn>
...
function KindGrid({ onPick }: { onPick: (kind: PickerStep) => void }) {
	const options: Array<{
	...
	}> = [
		{
			kind: "mcp",
			label: "MCP server",
			sub: "Tools exposed by an MCP server you've connected.",
		},
		{
			kind: "local",
			label: "Local tool",
			sub: "Built-in tools registered with the runner.",
		},
		{
			kind: "agent",
			label: "Another agent",
			sub: "Invoke another saved agent like a tool.",
		},
		{
			kind: "http",
			label: "HTTP endpoint",
			sub: "A GET/POST URL surfaced to the model as a tool.",
		},
	];
	return (
		<div style={{ padding: 6 }}>
...
function MCPSubPicker({
	...
	const quickPicks = useMemo<ScannedMcpEntry[]>(() => {
		...
		if (rootManifest) {
			for (const entry of scanMcpEntries(rootManifest))
