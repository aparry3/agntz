...
export function snapshotCards(
	...
	if (kind === "llm") {
		...
		return [
			...
			},
			{
				...
				sub:
					tools
						.map((t) => describe((t as { kind?: string }).kind, "tool"))
						.join(" · ") || "none",
			},
			{
				label: "Output",
	...
	if (kind === "sequential") {
		...
		const loop = parsed.until
			? `loops until ${describe(parsed.until)} · max ${describe(parsed.maxIterations, "?")}`
			: "linear · no loop";
		return [
		...
		return [
			{
				...
				value: "parallel",
				mono: true,
				sub: `${branches.length} branches run together`,
			},
			{
				label: "Branches",
	...
	if (kind === "tool") {
		...
		return [
			...
			{
				label: "Tool",
				value: describe(tool.name, "?"),
				mono: true,
				sub: describe(tool.kind, "—"),
			},
			{
				label: "Inputs",
				value: String(inputs.length),
	...
	return [
		...
		{
			label: "Id",
			value: describe(parsed.id),
			mono: true,
			sub: describe(parsed.name),
		},
	];
}
