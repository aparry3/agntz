		...
		>
			...
			<div style={{ padding: "14px 16px" }}>
				...
				>
					<Stat
						label="Started"
						value={new Date(span.startedAt).toLocaleString()}
					/>
					<Stat
						label="Duration"
						value={span.durationMs === null ? "—" : `${span.durationMs}ms`}
					/>
					...
					{tokens && (
						<>
							...
							<Stat
								...
								value={tokens.completion.toLocaleString()}
							/>
						</>
					)}
					...
					{toolDurationMs !== null && (
						<Stat label="Tool duration" value={`${toolDurationMs}ms`} />
					)}
					{span.costUsd !== null && (
						<Stat label="Cost" value={`$${span.costUsd.toFixed(6)}`} />
					)}
	...
	return (
		...
		>
			...
			<Mono
				size={11.5}
				color={ag.danger}
				style={{ lineHeight: 1.5, display: "block", whiteSpace: "pre-wrap" }}
			>
				{body}
			</Mono>
		</div>
