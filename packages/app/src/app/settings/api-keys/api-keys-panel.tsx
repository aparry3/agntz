...
export function ApiKeysPanel() {
	...
	async function revoke(id: string) {
		if (
			!confirm(
				"Revoke this key? Any apps using it will stop working immediately.",
			)
		)
			return;
	...
	return (
		<div className="space-y-6">
			<form
				onSubmit={createKey}
				className="flex gap-2 rounded-xl border border-stone-200 bg-white p-4"
			>
				<input
					value={name}
			...
			{revealKey && (
				<div className="rounded-xl border border-amber-300 bg-amber-50 p-4">
					...
					<div className="mb-2 text-xs text-amber-800">
						Copy and store it somewhere safe. Closing this banner clears it from
						the page.
					</div>
						...
						{keys?.map((k) => (
							<tr key={k.id}>
								...
								<td className="px-4 py-3">
									{k.revokedAt ? (
										<span className="rounded bg-stone-200 px-2 py-0.5 text-xs text-zinc-600">
											Revoked
										</span>
									) : (
