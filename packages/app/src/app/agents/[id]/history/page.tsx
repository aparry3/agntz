...
import {
	...
	formatMonthDay,
	relativeWhen,
} from "@/components/v3/history/format";
import { snapshotCards } from "@/components/v3/history/snapshot-cards";
import {
	type DiffLine,
	diffLines,
	diffStat,
} from "@/components/v3/history/yaml-diff";
...
export default function AgentHistoryPage() {
	...
	const reload = useCallback(async () => {
		...
		try {
			...
			if (!agentRes.ok)
				throw new Error(`Failed to load agent (HTTP ${agentRes.status})`);
			if (!versionsRes.ok)
				throw new Error(`Failed to load versions (HTTP ${versionsRes.status})`);
			const agent = await agentRes.json();
			const list = await versionsRes.json();
			const versionList = (
				Array.isArray(list) ? list : (list.versions ?? [])
			) as VersionSummary[];
			...
			if (versionList.length > 0) {
				const aliased = versionList.find(
					(v, i) => i > 0 && v.aliases.length > 0,
				);
	...
	useEffect(() => {
		...
		let cancelled = false;
		fetch(
			`/api/agents/${encodeURIComponent(id)}/versions/${encodeURIComponent(selectedId)}`,
		)
			.then((r) =>
			...
			.then((agent) => {
				if (cancelled) return;
				setSelectedManifest(
					(agent?.metadata?.manifest as string | undefined) ?? "",
				);
			})
			.catch((err) => {
				if (!cancelled)
	...
	const isSelectedCurrent =
		...
		selectedVersion.createdAt === currentVersion?.createdAt;
	...
	const isPipelineKind =
		parsedSelected?.kind === "sequential" ||
		parsedSelected?.kind === "parallel";
	...
	const handleDuplicate = () => {
		...
		try {
			sessionStorage.setItem("agntz.duplicateManifest", selectedManifest);
		} catch {
			/* ignore */
		}
	...
	const handleAddAlias = async (alias: string) => {
		...
		try {
			const res = await fetch(
				`/api/agents/${encodeURIComponent(id)}/aliases/${encodeURIComponent(alias)}`,
				{
					method: "PUT",
	...
	return (
		...
		>
			<HistoryHeader
				...
				kindTag={
					<Tag
						bg={isPipelineKind ? ag.purpleBg : ag.blueBg}
						color={isPipelineKind ? ag.purple : ag.blue}
						mono
					>
						{isPipelineKind
							? String(parsedSelected?.kind ?? "Pipeline")
							: "LLM"}
					</Tag>
				}
				...
				actions={
					<>
						...
						<Btn
							...
							disabled={
								!selectedVersion || isSelectedCurrent || pendingAction !== null
							}
							title={
								isSelectedCurrent
						...
						{view === "diff" ? (
							...
							<>
								{cards.length > 0 && (
									...
									>
										{cards.map((c) => (
											<SnapCardView key={c.label} card={c} />
										))}
									</div>
								)}
								<YamlPanel
				...
				<div style={{ minWidth: 0, flex: 1 }}>
					...
					>
						...
						<Mono size={11} color={ag.muted}>
							{versionsCount} version{versionsCount === 1 ? "" : "s"} · oldest{" "}
							{oldestDate}
						</Mono>
					</div>
				</div>
...
function VersionsRail({
	...
	const groups: Array<{ label: string; rows: VersionSummary[] }> =
		useMemo(() => {
		...
		>
			...
			>
				<FilterPill on={filter === "all"} onClick={() => onChangeFilter("all")}>
					All{" "}
					<Mono color={ag.muted} size={10.5} style={{ marginLeft: 4 }}>
						{totalCount}
					</Mono>
				</FilterPill>
				<FilterPill
					on={filter === "aliased"}
					onClick={() => onChangeFilter("aliased")}
				>
					Aliased{" "}
					<Mono color={ag.muted} size={10.5} style={{ marginLeft: 4 }}>
						{aliasedCount}
	...
	return (
		...
		>
			...
			<div style={{ flex: 1, minWidth: 0 }}>
				...
				>
					<Mono size={11.5} color={ag.ink} style={{ fontWeight: 500 }}>
					...
					<CopyButton
						...
						text={
							primaryAlias
								? `${agentId}@${primaryAlias}`
								: `${agentId}@${entry.createdAt}`
						}
				...
				<Mono
					...
					color={ag.muted}
					style={{
						marginTop: entry.aliases.length > 0 ? 4 : 2,
						display: "block",
						overflow: "hidden",
...
function SelectedVersionMeta({
	...
	return (
		<div
			style={{ borderBottom: `1px solid ${ag.line2}`, background: ag.surface }}
		>
			<div
				style={{
					padding: "12px 24px",
			...
			>
				...
				{diffStat && (
					...
					>
						...
						<Tag bg={H.addBg} color={H.addText} mono>
							+{diffStat.add}
						</Tag>
						<Tag bg={H.remBg} color={H.remText} mono>
							−{diffStat.rem}
						</Tag>
					</div>
				)}
				...
				>
					...
					{entry.aliases.length > 0 && (
						<Mono size={10.5} color={ag.muted} style={{ marginLeft: 4 }}>
							call{" "}
							<span style={{ color: ag.ink }}>
								{agentId}@{entry.aliases[0]}
							</span>{" "}
							to run this version
						</Mono>
					)}
...
function EmptyDiff() {
	return (
		...
		>
			<I.Eye size={12} />
			This is the current version — pick an older version on the left to see how
			it differs.
		</div>
	);
}
...
function DiffPanel({
	lines,
	label,
}: { lines: DiffLine[]; label?: React.ReactNode }) {
	let leftNo = 0;
	let rightNo = 0;
	return (
		<div
		...
		>
			...
			<div>
				{lines.length === 0 && (
					<div style={{ padding: "12px 16px", color: ag.muted, fontSize: 11 }}>
						No differences.
					</div>
				)}
				{lines.map(([kind, text], i) => {
					...
					const bg =
						kind === "+" ? H.addBg : kind === "-" ? H.remBg : "transparent";
					const gutter =
						kind === "+"
							? H.addGutter
							: kind === "-"
								...
								: "transparent";
					const fg =
						kind === "+" ? H.addText : kind === "-" ? H.remText : ag.ink;
					return (
...
function SnapCardView({
	card,
}: { card: ReturnType<typeof snapshotCards>[number] }) {
	return (
		<div
			style={{
				padding: "10px 12px",
...
function AliasChip({
	...
	return (
		...
		>
			<span style={{ color: ag.muted, marginRight: 1 }}>@</span>
			{alias}
			<button
				type="button"
				onClick={(e) => {
