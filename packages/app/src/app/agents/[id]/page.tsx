...
} from "@/components/v3/editor/pipeline-view";
import {
...
export default function AgentEditorPage() {
	...
	const isPipeline =
		parsed?.kind === "sequential" || parsed?.kind === "parallel";
	const manifestId = typeof parsed?.id === "string" ? parsed.id : id;
	const manifestName =
		typeof parsed?.name === "string" ? parsed.name : manifestId;
			...
			secondaryActions={
				<>
					<Btn
						...
						onClick={() =>
							router.push(`/agents/${encodeURIComponent(manifestId)}/history`)
						}
					>
						History
					</Btn>
