...
export default function SessionsPage() {
	...
	const agentIds = useMemo(
		() =>
			Array.from(
				new Set(
					sessions.map((s) => s.agentId).filter((a): a is string => Boolean(a)),
				),
			).sort(),
		[sessions],
	);
		...
		>
			...
			>
				...
				<Mono size={11} color={ag.muted}>
					{loading
						? "loading…"
						: `${filtered.length} sessions · sort: updated ↓`}
				</Mono>
			</div>
...
function SessionRow({
	session,
	isLast,
}: { session: Session; isLast: boolean }) {
	...
	return (
		...
		>
			...
			<div style={{ fontSize: 12.5, color: ag.muted }}>
				Conversation sessions appear here once an agent starts handling stateful
				chats.
			</div>
		</div>
	);
