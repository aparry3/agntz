...
export function useCatalog(): Catalog {
	...
	>({});
	const inflightMcpTools = useRef<
		Record<string, Promise<string[]> | undefined>
	>({});
	const [modelsByProvider, setModelsByProvider] = useState<
		Record<string, ProviderModelsResult | undefined>
	>({});
	const inflightModels = useRef<
		Record<string, Promise<ProviderModelsResult> | undefined>
	>({});
	...
	useEffect(() => {
		...
		Promise.all([
			...
			([providersData, toolsData, agentsData, serversData, secretsData]) => {
				if (cancelled) return;

				setProviders(Array.isArray(providersData) ? providersData : []);

				const toolsArr: ToolInfoFromApi[] = Array.isArray(toolsData)
					? toolsData
					: [];
				setTools(
					toolsArr.map((t) => ({
				...
				const agentsArr: AgentFromApi[] = Array.isArray(agentsData)
					? agentsData
					: [];
				setAgents(
					agentsArr.map((a) => ({
				...
				const secretsArr: SecretFromApi[] = Array.isArray(secretsData)
					? secretsData
					: [];
				setSecrets(
					secretsArr.map((s) => ({
