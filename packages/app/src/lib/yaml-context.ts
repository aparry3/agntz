...
export function mcpServerInScope(ctx: YamlContext): string | null {
	if (
		ctx.parentSiblings.kind === "mcp" &&
		typeof ctx.parentSiblings.server === "string"
	) {
		return ctx.parentSiblings.server;
	}
	if (
		ctx.scopeSiblings.kind === "mcp" &&
		typeof ctx.scopeSiblings.server === "string"
	) {
