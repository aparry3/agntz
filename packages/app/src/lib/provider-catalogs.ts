...
const fetchOpenRouter: Fetcher = async () => {
	...
	return (data.data ?? []).map((m) => {
		...
		const promptPer1M = promptStr
			? Number.parseFloat(promptStr) * 1_000_000
			: undefined;
		const completionPer1M = completionStr
			? Number.parseFloat(completionStr) * 1_000_000
			: undefined;
		const tags: string[] = [];
...
const FETCHERS: Record<string, { fetch: Fetcher; requiresKey: boolean }> = {
	...
	openai: {
		fetch: fetchOpenAICompatible("https://api.openai.com/v1/models"),
		requiresKey: true,
	},
	anthropic: { fetch: fetchAnthropic, requiresKey: true },
	google: { fetch: fetchGoogle, requiresKey: true },
	groq: {
		fetch: fetchOpenAICompatible("https://api.groq.com/openai/v1/models"),
		requiresKey: true,
	},
	xai: {
		fetch: fetchOpenAICompatible("https://api.x.ai/v1/models"),
		requiresKey: true,
	},
	mistral: {
		fetch: fetchOpenAICompatible("https://api.mistral.ai/v1/models"),
		requiresKey: true,
	},
	deepseek: {
