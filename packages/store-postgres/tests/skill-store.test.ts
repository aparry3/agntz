...
describe.skipIf(!hasDb)("PostgresStore SkillStore (integration)", () => {
	...
	it("isolates skills per user (same name allowed for different users)", async () => {
		...
		await storeA.putSkill(
			makeSkill({ name: "shared-name", description: "a-flavor" }),
		);
		await storeB.putSkill(
			makeSkill({ name: "shared-name", description: "b-flavor" }),
		);

		expect((await storeA.getSkill("shared-name"))?.description).toBe(
			"a-flavor",
		);
		expect((await storeB.getSkill("shared-name"))?.description).toBe(
			"b-flavor",
		);
