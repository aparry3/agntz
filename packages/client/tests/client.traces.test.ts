...
describe("AgntzClient.traces.list", () => {
  it("GET /traces with query params and Bearer", async () => {
    const mock = mockFetch(() => jsonResponse(200, { rows: [sampleSummary()] }));
    ...
    expect((mock.calls[0]!.init.headers as Record<string, string>).Authorization).toBe("Bearer k");
  });
...
describe("AgntzClient.traces.get", () => {
  ...
  it("URL-encodes the traceId", async () => {
    const mock = mockFetch(() => jsonResponse(200, { summary: sampleSummary(), spans: [] }));
    const client = new AgntzClient({ apiKey: "k", baseUrl: BASE, fetch: mock.fetch });
...
describe("AgntzClient.traces.stream", () => {
  ...
  it("yields live span-start / span-end / trace-done in order", async () => {
    ...
    const collected: TraceLiveEvent[] = [];
    for await (const ev of client.traces.stream("tr_1")) collected.push(ev);
    expect(collected.map((e) => e.type)).toEqual(["span-start", "span-end", "trace-done"]);
  });
});
