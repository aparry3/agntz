...
describe("AgntzClient.runs.start", () => {
	it("POST /runs with body and Bearer, returns the Run handle", async () => {
		...
		const out = await client.runs.start({
			agentId: "a1",
			input: { x: 1 },
			sessionId: "s",
		});
...
describe("AgntzClient.runs.stream", () => {
	...
	it("appends ?since=N when provided", async () => {
		...
		expect((mock.calls[0]?.init.headers as Record<string, string>).Accept).toBe(
			"text/event-stream",
		);
	});
	...
	it("deserializes `reply` events into typed MultiplexedRunEvent.reply", async () => {
		...
		expect(
			(replyEvents[0] as Extract<MultiplexedRunEvent, { type: "reply" }>).text,
		).toBe("still thinking...");
	});
});
...
describe("AgntzClient.runs.list", () => {
	it("calls GET /runs with no query params when filter is empty", async () => {
		const mock = mockFetch(() =>
			jsonResponse(200, { rows: [], cursor: undefined }),
		);
		const client = new AgntzClient({
			apiKey: "k",
			baseUrl: BASE,
