...
import type { StreamEvent, MultiplexedRunEvent } from "@agntz/client";
...
async function cmdRun(args: string[]): Promise<void> {
  ...
  if (positionals.length === 0) {
    fail("Usage: agntz run <path-or-id> [--input <text>] [--session <id>] [--stream]");
  }
  ...
  if (values.remote || (!isLocalShape && !values.local)) {
    await runHosted({ agentId: target, input: inputText, sessionId, stream: values.stream });
    return;
  }
  await runLocal({ target, input: inputText, sessionId, stream: values.stream });
}
...
async function runLocal(opts: {
  ...
  if (!agentId) {
    ...
    if (ids.length === 1) {
      ...
      fail(
        `Directory ${opts.target} contains ${ids.length} agents. ` +
          `Specify a file path or use 'agntz run <id>' against the hosted API.`,
      );
    }
  }
...
function printRunResult(result: {
  ...
  const out = typeof result.output === "string" ? result.output : formatJson(result.output);
...
function printStreamEvent(event: StreamEvent): void {
  switch (event.type) {
    ...
    case "complete": {
      const out =
        typeof event.output === "string" ? event.output : formatJson(event.output);
      process.stdout.write(`\n${out}\n`);
      return;
    }
    case "error":
...
async function resolveInput(flagInput: string | undefined, trailingInput: string): Promise<string> {
