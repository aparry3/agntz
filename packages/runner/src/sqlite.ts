 ...
 * ```
 */
export function sqliteStore(options: SqliteStoreOptions | string): UnifiedStore {
  const opts: SqliteStoreOptions = typeof options === "string" ? { path: options } : options;
  ...
  const admin = new SqliteStore({ path: opts.path, wal: opts.wal, verbose: opts.verbose });
