...
const PROMPT = 'What is the dominant color of this image? Answer with one word.';
...
export const multimodalUrl: TestDefinition = {
  ...
  async run(model, ctx) {
    ...
    if (!url) {
      return { ok: true, skip: 'set HARNESS_IMAGE_URL to a public image to test URL-based input' };
    }
    const result = await provider.generateText({
      model: modelConfig(model),
      messages: [
