...
function buildMarkdown(input: ReportInput): string {
  ...
  lines.push(`- **Matrix:** ${input.matrix.length} models · ${ALL_CAPABILITIES.length} capability dimensions`);
  lines.push('');
  lines.push('## Summary');
  lines.push('');
  ...
  const failing = input.results.filter((r) => r.bucket === 'SDK_ERROR' || r.bucket === 'UNEXPECTED_UNSUPPORTED');
  if (failing.length > 0) {
    lines.push('');
...
function buildMatrixTable(matrix: readonly ProviderModelEntry[]): string {
  ...
  return [headers, sep, ...rows].map((row) => `| ${row.join(' | ')} |`).join('\n');
}
...
function countByBucket(results: readonly TestResult[]): Record<ResultBucket, number> {
