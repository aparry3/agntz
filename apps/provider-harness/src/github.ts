...
export async function maybeOpenIssue(opts: GithubReportOptions): Promise<void> {
  ...
  if (opts.dryRun) {
    ...
    console.log(`    gh issue create --title ${JSON.stringify(title)} --label ${LABEL} --body <${body.length} chars>`);
    return;
  }

  if (!(await ghAvailable())) {
    console.log('  --report-github: `gh` CLI not found on PATH; skipping issue creation.');
    return;
  }
  ...
  try {
    ...
    console.log(`  --report-github: failed to open issue: ${(err as Error).message}`);
  }
}
...
function buildIssueBody(failures: readonly TestResult[], markdownPath: string): string {
  const lines: string[] = [];
